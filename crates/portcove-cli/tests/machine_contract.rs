...
fn json_and_jsonl_preserve_machine_error_fields_and_share_core_presentation() {
    ...
    assert_eq!(stream.status.code(), Some(3));
    let plain = json_stdout(&plain);
    ...
    .unwrap();
    assert_eq!(plain["schema_version"], 54);
    assert_eq!(stream["schema_version"], 54);
    ...
    assert_eq!(plain["error"]["details"], serde_json::json!({}));
    assert_eq!(plain["error"]["presentation"]["mutation_state"], "unknown");
    assert_eq!(
        plain["error"]["presentation"]["presentation_key"],
        "operation_unsupported"
    );
}

#[test]
fn human_failures_keep_paths_and_credentials_out_of_primary_copy() {
    let temporary = tempfile::tempdir().unwrap();
    let library = temporary.path().join("library");
    let private_id = "fixture-token=owned-private";
    let plain = portcove(&library, &["catalog", "show", private_id]);
    assert_eq!(plain.status.code(), Some(4));
    let plain = String::from_utf8(plain.stderr).unwrap();
    assert!(plain.contains("A required item is no longer available."));
    assert!(!plain.contains(private_id));
    assert!(!plain.contains("No files were changed"));
    let technical = portcove(
        &library,
        &["--technical-details", "catalog", "show", private_id],
    );
    assert_eq!(technical.status.code(), Some(4));
    let technical = String::from_utf8(technical.stderr).unwrap();
...
fn activity_history_plain_output_keeps_failure_details_private() {
    ...
    let record = &before["data"]["records"][0];
...
fn activity_history_keeps_machine_records_unchanged_across_formats() {
    ...
    assert!(!failed.status.success());
    let before = json_stdout(&portcove(&library, &["--json", "activity"]));
    let id = before["data"]["records"][0]["id"].as_str().unwrap();
...
fn preparation_plan_reports_missing_inputs_without_starting_work() {
    ...
    assert!(activities["data"]["records"].as_array().unwrap().is_empty());
    assert_eq!(activities["data"]["active_and_actionable_complete"], true);
    assert_eq!(activities["data"]["terminal_history_complete"], true);
...
fn exported_source_assessment_separates_facts_without_opening_library() {
    ...
    let response = json_stdout(&output);
    assert_eq!(response["schema_version"], 54);
...
fn cancellation_from_another_cli_stops_discovery_with_a_durable_cancelled_result() {
    ...
    assert_eq!(ledger["data"]["records"][0]["id"], id);
    assert_eq!(ledger["data"]["records"][0]["status"], "cancelled");
...
fn saved_game_file_roots_survive_unavailability_and_relink_by_stable_identity() {
    ...
    let added = json_stdout(&portcove(
        &library,
        &["--json", "source", "roots", "add", first.to_str().unwrap()],
    ));
    assert_eq!(added["command"], "source.roots.add");
    assert_eq!(added["data"]["availability"], "available");
    let first_id = added["data"]["id"].as_str().unwrap().to_owned();
    let duplicate = json_stdout(&portcove(
        &library,
        &["--json", "source", "roots", "add", first.to_str().unwrap()],
    ));
    assert_eq!(duplicate["data"]["id"], first_id);

    std::fs::remove_dir(&first).unwrap();
    let unavailable = json_stdout(&portcove(&library, &["--json", "source", "roots", "list"]));
    assert_eq!(unavailable["data"][0]["id"], first_id);
    assert_eq!(unavailable["data"][0]["availability"], "unavailable");

    let relinked = json_stdout(&portcove(
        &library,
        &[
            "--json",
            "source",
    ...
    assert_eq!(relinked["command"], "source.roots.relink");
    assert_eq!(relinked["data"]["id"], first_id);
    assert_eq!(relinked["data"]["availability"], "available");
    ...
    assert_eq!(scan["data"]["report"]["entries_examined"], 0);
    assert_eq!(
        scan["data"]["report"]["limits_reached"],
        serde_json::json!([])
    );

    let snapshot = json_stdout(&portcove(
        &library,
    ...
    let streamed = portcove(&library, &["--jsonl", "source", "roots", "scan"]);
    assert!(streamed.status.success(), "{streamed:?}");
    let events = std::str::from_utf8(&streamed.stdout)
        .unwrap()
        .lines()
        .map(|line| serde_json::from_str::<Value>(line).unwrap())
        .collect::<Vec<_>>();
    ...
    assert_eq!(stale["data"]["freshness"], "inputs_changed");

    let removed = json_stdout(&portcove(
        &library,
        &["--json", "source", "roots", "remove", &first_id],
    ));
    assert_eq!(removed["data"]["root_id"], first_id);
    assert_eq!(removed["data"]["removed"], true);
    assert_eq!(
        json_stdout(&portcove(&library, &["--json", "source", "roots", "list"],))["data"],
        serde_json::json!([])
    );
}
...
fn source_inbox_controls_share_stable_scan_and_import_activity_ids() {
    ...
    assert_eq!(scan["schema_version"], 54);
    ...
    assert_eq!(activity["data"]["records"][0]["id"], scan_id);
    assert_eq!(
        activity["data"]["records"][0]["operation"],
        "discover_sources"
    );
...
fn port_status_has_human_output_snapshot() {
    ...
    assert!(status.contains("launch: waiting (not_installed)"));
    ...
    assert_eq!(machine["schema_version"], 54);
    assert_eq!(machine["data"]["port_actions"][1]["action"], "launch");
    assert_eq!(
        machine["data"]["port_actions"][1]["availability"],
        "waiting"
    );
}
...
fn capabilities_are_one_clean_versioned_json_document() {
    ...
    assert_eq!(response["data"]["schema_version"], 54);
    ...
    assert!(
        response["data"]["commands"]
            .as_array()
            .unwrap()
            .contains(&serde_json::json!("source.roots"))
    );
    assert!(
        response["data"]["commands"]
            .as_array()
            .unwrap()
            .contains(&serde_json::json!("source.roots.scan"))
    );
    assert_eq!(
