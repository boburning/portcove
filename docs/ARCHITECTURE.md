...
statuses, registered sources, and recent activity. The Tauri adapter creates one request-scoped
service, including its normal recovery pass, and does not retain a second service or cancellation
owner. Host and repair diagnostics remain a separate generation-bound request so a slow or failed
probe cannot withhold essential library state. React coalesces refresh invalidations, retains stale
snapshots explicitly after failure, and treats events as hints: SQLite readback remains authoritative,
active work is polled without overlap, and idle polling backs off further while the window is hidden.
Listener setup and cleanup failures degrade to visible manual refresh plus polling rather than an
unhandled rejection or a second durable state authority.
Each React effect lifetime owns fresh workspace, diagnostic, and activity coordinators. Strict Mode
setup replay and library changes dispose the old generation, settle its waiters as disposed, and
cannot reuse or reopen a coordinator whose callbacks belong to the prior lifetime.
