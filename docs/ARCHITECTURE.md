...
update-state directory without owner write permission, a bounded update-state
filesystem with zero available bytes, and a bounded stable-AppImage filesystem with
zero available bytes each fail closed while the predecessor restarts. The state
fixture leaves every pending byte unchanged. The AppImage fixture leaves the stable
predecessor and staged candidate exact while advancing only its isolated apply copy
to a failed launch with the exact attempted replacement identity. It then exercises
the explicit retry transition, which clears that failed attempt while retaining the
intent and staged candidate. Each full-filesystem fixture uses a disposable
`tmpfs`, fills it to an independently measured zero-byte boundary, removes the
bounded filler, proves writes recover, and unmounts it. It restores only the fixture
schema or storage condition needed to continue. It then holds the shared lock in
another process, verifies the helper leaves the stable AppImage and pending state
unchanged while blocked, and releases the peer so the same helper can
...
Catalog `runtime_mutable_file_patterns` reuses that anchored filename syntax for narrowly reviewed, disposable files created inside the managed runtime working directory. Core projects both pattern classes into the existing manifest `mutable_file_patterns`, with their separately resolved directories, so immutable verification has one fail-closed matching boundary. Only `persistent_file_patterns` participates in enumeration, backup, restore and migration; runtime-only matches never become player-data ownership. Catalog validation refuses unsupported adapters, overlapping declarations, executables, source materialization, Portcove metadata, exact runtime paths and persistent data, while manifest construction and import repeat the resolved executable/runtime checks. An absent field preserves prior catalog serialization and existing manifest schemas require no new authority. External per-user paths remain outside this contract.
...
Core also owns the read-only output-destination preview, capacity/ownership assessment, state-bound fingerprint, one-use authorization, port locking, and apply-time revalidation. CLI and Tauri translate the same preview and mutation results. New desktop storage commands bind each request to the active bootstrap generation, while React holds only the typed path entry and transient review state; a library, port, path, or newer-request change discards an old response. The Settings surface reviews whole-library switching separately, and a game page labels its per-port choice “Install folder” and states that it changes future placement without relocating recorded installs.

Management operations create a running activity before work and finish it as succeeded or failed without replacing the command's primary result. Best-effort progress uses a versioned core event envelope containing that activity UUID, a per-operation sequence, millisecond timestamp, typed target, optional parent operation, and terminal result. Nested work receives its own ID and names the parent. SQLite activity is authoritative after disconnect or restart; event delivery is never treated as durable truth. Update snapshots likewise come from the core, allowing a CLI check to repopulate the desktop after restart; consumers validate the snapshot's installed artifact digest, display version, and channel before presenting it as current. Status acquisition is set-oriented and scoped to the requested port identities: settings, related installs, launch history, and update snapshots use four parameterized queries per chunk, capped at 4,000 ports and reduced to the live SQLite connection's variable limit, preserve caller order, and decode no unrelated rows. A single-port service read likewise loads only the source profiles required by that port's current or retained definition. Catalog-wide status uses the same path and reassembles results in catalog order without writing default rows. Required source identities are hashed once per profile during that status request and shared by every installed dependent. Uninstalled registrations are reported as `not_checked` instead of causing catalog browsing to hash source files. Debug diagnostics record both query and decoded-row counts; scale tests cover 250, 500, 1,000, and 5,000 requested ports plus a one-port read beside 5,000 malformed unrelated records.

Desktop workspace hydration is one generation-bound core read model containing the catalog,
statuses, registered sources, and recent activity. Core loads and validates the immutable catalog
for that request immediately before starting one short deferred SQLite read transaction. Sources,
settings, related installs, launch history, update snapshots, and the included activity window all
come from that transaction. A persistent observing connection compares its own `data_version`
before catalog acquisition, at the established read snapshot, and after post-transaction hydration;
a concurrent catalog-selection, policy, or library write retries the complete acquisition once.
Continued churn returns an explicit stale/retry conflict rather than a mixed snapshot. The transaction
is committed before retained manifests, source files, or definition eligibility are inspected, so
filesystem work and validation never extend the database snapshot; a later database write during
that work is detected by the final comparison. Filesystem health remains an observation made during
hydration, not transactional authority, and every later mutation still repeats its operation-time
validation. The Tauri adapter creates one request-scoped service, including its normal recovery pass,
and does not retain a second service or cancellation owner. Host and repair diagnostics remain a
separate generation-bound request so a slow or failed probe cannot withhold essential library state.
React coalesces refresh invalidations, retains stale snapshots explicitly after failure, and treats
events as hints: SQLite readback remains authoritative, active work is polled without overlap, and
idle polling backs off further while the window is hidden. Listener setup and cleanup failures
degrade to visible manual refresh plus polling rather than an unhandled rejection or a second durable
state authority. Each React effect lifetime owns fresh workspace, diagnostic, and activity
coordinators. Strict Mode setup replay and library changes dispose the old generation, settle its
...
thirty seconds while hidden. A detected commit invalidates diagnostics and coalesces one essential
workspace read. Foreground restoration always requests an essential read, and visible sessions run a
sixty-second full fallback so non-database catalog or source-file observations also converge. Library
replacement creates a new observer baseline; generation checks reject every old result. These are
readback hints only and never mutation authority.

Visual operation events update React state immediately, while their durable activity readbacks share
one scheduler with polling and foreground hints. Start and finish boundaries are prompt; progress and
message events admit at most one new read per 500 milliseconds without resetting the deadline, and
the coalescer retains at most one follow-up behind an in-flight read. Diagnostic consumers share the
last completed report together with its never-loaded, loading, stale, failed, or fresh presentation.
...
Library import reads a metadata export and an explicitly selected copy of its content trees without opening or migrating the input database. The destination must be new or contain an empty Portcove library; merging into existing domain state is rejected. Core reviews capacity, portable paths, source references, and active/previous/staged identity, then requires the content-bound plan again under an exclusive destination lease. A bounded destination-owned import journal is also the open gate and is published before database initialization or copying. Metadata hydration uses one SQLite transaction and the same source/install writers as ordinary registration. The shared transfer verifier checks all copied bytes and manifest identities; import additionally rebuilds the current platform executable, mutable-path, and critical-file policy for comparison with each restored manifest. An exact retained definition that records prior authenticated admission remains the authority for its own installed version, including a successor port absent from the running client's embedded catalog. Export first requires the retained identity to match the source library's persisted active or rollback selection transaction, then binds the selection role plus install, manifest and selection identities to a host-local key in operating-system secure storage. Planning and every recovery read verify that receipt before using the digest-bound retained graph; this deliberately proves same-host continuity rather than inventing cross-device trust. Import restores that admitted active/rollback context into the otherwise empty destination before comparing its metadata. A bare definition snapshot, copied catalog, unknown port ID, structurally valid admission object, or self-consistent content hashes cannot create admission, and an unadmitted retained contract for an embedded port must still match the embedded installed-code contract. This preserves frozen source, execution, persistence, artwork and qualification identities without treating the current definition as permission to rewrite historical installs. It establishes consistency with a trusted local backup, not independent publisher authenticity for that backup.

Only a verified import changes its journal to published and becomes openable. The verified and published phases require a host-authenticated proof bound to the transfer and complete reviewed plan, so changing the mutable phase enum cannot skip copy, metadata, inventory, database, artwork or execution-contract verification. Recovery after publication finalizes bookkeeping without reading offline input or replaying old saves. Terminal activity is idempotent, and an interrupted abort cannot later publish as success. Completed journals move under recovery so normal opens do not parse a full historical inventory. CLI abort preserves and gates an incomplete destination; a subsequent import uses another empty root. Desktop Settings restores into the currently configured empty library, releases cached handles for the core operation, obtains native confirmation of the trusted backup and destination, and reopens the same root. Original source references retain their locations and must pass current profile validation or explicit relinking before use. There is no new crate boundary or parallel library authority.
...
The desktop process shares one release provider across Tauri commands. Successful release and asset selections are cached in memory for five minutes, but every reuse first revalidates current GitHub or GitLab repository metadata; an observed archive rejects the cached selection, and a temporary network failure remains a network failure. Those selector caches retain at most 256 entries. Provider JSON is streamed under a 4 MiB per-response limit and parsed before it may replace the library-scoped SQLite conditional cache, which opportunistically retains at most 512 response bodies, 64 MiB, and seven days. `ETag` and `Last-Modified` requests therefore reuse only the last semantically valid body on `304 Not Modified`. Metadata and authentication calls have ten-second connection, fifteen-second read-idle, and sixty-second overall bounds. Provider metadata redirects are rejected; unauthenticated checksum downloads may follow at most five redirects and are streamed under a 1 MiB limit. Checksum and artifact downloads have connection and read-idle protection but no short total deadline, so a progressing large transfer remains possible and cancellable. GitHub and GitLab release discovery reads at most ten 100-item pages. GitLab package discovery reads the same maximum, then checks at most sixteen packages whose version matches the selected release, with four package-file list requests in flight and at most five 100-file pages per package. Equally scored runnable assets or package links fail as ambiguous, and an aggregate checksum file must name the exact selected asset; only its exact `.sha256` sidecar may contain a bare digest. Core bulk update checking admits at most four provider resolutions at once, returns results in the caller's order, records each activity independently, and never retries a rate-limited item implicitly; CLI and Tauri only translate those outcomes. React generation-gates overlapping refreshes and tracks pending commands independently, so reverse completion cannot replace newer data or clear an unrelated busy state. Gamepad navigation is scoped to the top modal and a single Back stack closes command palette, adoption, then port detail in visual order. A top-level render boundary reports the component failure to Tauri and presents a reload surface without claiming domain failure. If library opening, migration recovery, or release-provider construction fails, Tauri keeps a typed failed bootstrap state: every domain command returns that error and React shows a retry/recovery surface without exposing library actions.
...
A conditional SQLite write serializes cancellation against the end of preparation. Requests accepted before that boundary stop at cooperative checkpoints. Network waits, download reads, source/download hashing, archive preflight, and archive entry writes check between bounded steps. The awaited blocking archive worker stops before cancellation cleanup can remove its private tree or report completion; conversion, setup, and managed builder workers still finish atomically before their next checkpoint and are never abandoned. Install closes cancellation before recording the recoverable `prepared` intent. Retained/staged activation closes it before collecting data or changing pointers. Launch uses the same activity UUID and closes cancellation as its final pre-spawn action; hashing checks incrementally, and conversion, setup, mutable-data synchronization, manifest refresh, and source revalidation each check before the child boundary. A CLI signal before that boundary requests durable cancellation; after it, the signal is forwarded to the child process group. Publication, restore, migration, and library transfers remain outside the cancellable region.

Accepted install cancellation removes only its identified private staging tree and intent; failed private cleanup is retained for review without permitting publication. Reviewed retained-work cleanup binds the exact inventory fingerprint into a same-volume operation quarantine and atomically isolates that tree before deletion. Startup retries only that accepted snapshot; files later created at the original staging path remain for a new review. Startup holds the activity and port locks before recovering interrupted preparation and preserves canonical user data. Prepared or published work follows the existing lifecycle recovery path. The activity ledger records a distinct `cancelled` terminal status before cancellation is returned, and schema-2 operation events distinguish it from success and failure. CLI API schema 6 exposes cancellation state, request results, and error/exit code `cancelled`/130. No second durable operation authority or crate boundary is introduced.
...
Device authorization uses a public GitHub App client ID and stores the resulting user token only after GitHub validates it. A personal token follows the same validation and storage path. Every token, device-flow, or logout start advances one provider-owned intent identity; a completion may touch secure storage only while that identity is still current, without holding the intent lock across a network wait. Starting newer authentication work invalidates older device sessions, while abandoned sessions are additionally limited to eight entries and fifteen minutes. The GUI and CLI expose authentication status and rate-limit metadata without exposing credential material.
...
Lifecycle startup and explicit orphan discovery use the core journal under the existing source-dependent or per-port locks. The initial journal inventory identifies candidate IDs; recovery reloads each record after acquiring its lock and uses its current phase and durable fields. A removed record is not recreated, and a record whose kind, port owner, or creation identity changed is left for a later correctly locked pass. Unchanged failed retries retain the original cause and latest bounded retry diagnostic without rewriting the journal. Core read-only service construction leaves cancellation and lifecycle recovery untouched. Desktop startup and library-generation initialization recover pending work; ordinary status/workspace reads do not. When the desktop change observer sees an external library commit, or the visible periodic reconciliation is due, a separate generation-bound discovery call retries newly orphaned CLI work before refreshing the view. Live-worker locks still exclude recovery. CLI observation commands open without advancing recovery, while mutating commands retain startup recovery.
...
focused hook tests preserve that composition contract. App imports the feature
directly, and the former root `use-portcove.ts` compatibility module is removed
