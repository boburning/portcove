...
- `html[data-theme]` is the runtime theme marker and activates Tailwind `dark:` variants. `html[dir]` and Base UI's `DirectionProvider` share direction with portaled controls. The current product direction remains `ltr`; locale-owned switching and translated copy are separate outcomes.
