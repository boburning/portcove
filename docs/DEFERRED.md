...
- Impact: none on the blocking `just check` or `just audit` definitions. Rust duplication remains visible through rscheck, and TypeScript duplication remains covered by Fallow. `just deep` still attempts semdup and reports its absence as advisory. The manual Ubuntu 24.04 Deep quality workflow has proven the pinned semdup runtime through indexing and model startup; its first uncached scan reached 256 of 640 embeddings before the original 45-minute budget expired. The workflow now caches the model and corpus and allows a 90-minute cold run. A completed hosted semantic report remains required before this fallback is considered fully proven.
