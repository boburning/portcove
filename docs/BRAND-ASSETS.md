...
`apps/desktop/assets/brand/manifest.json` is the machine-readable integrity contract for approved masters and runtime derivatives. The release check verifies every listed PNG's SHA-256, dimensions, 8-bit color mode, unique identity, and repository-contained path. Update the manifest only when the project owner approves a replacement or new production asset.
...
A small Blender source is the recommended long-term authority for the turnaround. Build one mirrored low-poly half mesh for the shell and paired appendages, use named flat materials for the canonical color roles, parent each attached lid to a simple outer hinge, and keep the legs and claws as separate low-complexity objects. Save the editable `.blend` source and export a reviewable GLB. Add only the articulation needed for lid expression and basic poses; a full production rig is unnecessary. The approved front master remains the measurement reference, and both approved three-quarter masters constrain depth and attachment geometry.
