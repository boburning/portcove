...
Required pull-request CI has a five-minute warm-cache target, measured from run creation to its terminal result. The Windows Rust lane installs only Node and the pinned Rust toolchain, and Clippy's all-target compilation replaces a duplicate standalone `cargo check`; format, Clippy, and the complete workspace tests remain required. The Linux quality lane likewise avoids installing pnpm because it invokes Node and Rust tools directly. These boundaries are enforced by `scripts/ci-workflow.test.mjs` so unused setup cannot silently return to the critical path.

Frontend pull requests use the required GitHub dependency-review check to block newly introduced high-severity vulnerabilities. Dependabot alerts and automated security fixes provide continuous repository-wide npm monitoring. The frontend build lane therefore does not make a second live request to npm's advisory endpoint on every commit; that duplicate request added no change-specific coverage and could hold all otherwise-passing checks open for repeated network timeouts. Frozen lockfile installation, production build, tests, and Fallow remain required.
