...
Required pull-request CI has a five-minute warm-cache target, measured from run creation to its terminal result. The Windows Rust suite is exhaustively partitioned into service, recovery, remaining-core, and non-core workspace shards. The smallest shard then runs format and all-target Clippy on its already-built graph, while a lightweight Windows job covers development-storage behavior. The required `rust` job fails closed unless the complete matrix and Windows storage producer pass. Clippy's all-target compilation replaces a duplicate standalone `cargo check`; the Linux quality lane likewise avoids installing pnpm because it invokes Node and Rust tools directly. These boundaries are enforced by `scripts/ci-workflow.test.mjs` so missing shards, unused setup, or accidental serialization cannot silently return to the critical path.

The sharded layout's cold-cache baseline is [run 33832768415](https://github.com/boburning/portcove/actions/runs/33832768415): every required check passed and the longest lane completed in 7m45s while compiling the Windows graph and populating its shared cache. The five-minute target applies after that cache has been established; a lockfile or toolchain change is expected to pay the cold-build cost once.
