...
release record cannot be changed or omitted. The tool neither authenticates its
controller inputs nor signs TUF metadata, publishes, downloads or activates an
updater; the protected controller must verify inventories immediately before use
and the distinct TUF roles authenticate the generated bytes.
