# Deferred work (historical)

> **Historical evidence — 2026-09-03.** This is the superseded mutable
> documentation ledger. It is preserved for provenance, not current planning.
> Live deferred and blocked work belongs in the
> [Portcove Roadmap](https://github.com/users/boburning/projects/1).
...
- Automated Windows qualification is already recorded for the ports below, but hands-on qualification is still absent:
  ...
  - BattleShip
  - Paper Mario ReCut (`beta`)
  - Mega Man X6 Recompiled
  - Severed Chains (`rolling`)
  - Virtual Pro Wrestling 64: Recompiled
  - WWF WrestleMania 2000: Recompiled
  - Space Station Silicon Valley: Recompiled
  - Trouble Makers Recompiled
  - Dinosaur Planet: Recompiled
  - OpenGOAL: Jak and Daxter
  - OpenGOAL: Jak II
  - OpenGOAL: Jak 3

The embedded catalog's `automated_tested_platforms` and `manually_validated_platforms` fields are the machine-readable authority. Update those fields only after the matching evidence exists. The earlier baseline Portcove desktop/controller smoke was reported as passing on Windows; this item concerns the listed games, not that completed baseline. `scripts/qualification-report.mjs` now captures the exact CLI identity and core status, source, activity, storage, and backup evidence for an existing isolated library, with gameplay/audio/controller/save observations explicitly left unassessed. Use the procedure in `docs/RELEASING.md` before and after the hands-on session.
...
- Later live credential rejection: HTTP 401 exposed a reconnect dead end, corrected in `dd7195b`. Core retains the rejected credential source and Settings exposes recovery without deleting it or silently retrying anonymously. HTTP 401/500 and component regressions pass; a final native interrupted-flow/retry observation remains.
...
- Campaign evidence, 2026-09-03: an isolated native Linux Rust 1.88 toolchain under WSL Ubuntu 26.04 passed 237 core tests, 19 CLI unit tests, and 14 compiled CLI contract tests. This includes Unix-only symlink and process-lifetime cases. It does not qualify a Linux desktop bundle, graphics/audio/controller behavior, or any game's platform badge. macOS hardware remains unavailable in this session.
...
- Evidence refresh: 2026-09-03; all 70 direct GitHub repositories and the applicable direct non-GitHub release pages were rechecked during this campaign. RomM authority was confirmed as version 5.2.0 with host `/volume1/media/roms` mounted read-only at `/romm/library`. GitHub release checks used Portcove's normal resolver and fail-closed checksum rules.

Existing candidates with unresolved blockers:
...
| DKR-R | The newly supplied USA Rev 1 cartridge matches upstream SHA-1 `6d96743d46f8c0cd0edb0ec5600b003c89b93755` and now registers through the pinned source profile. Upstream still publishes only Linux x64/Steam Deck and Apple-silicon Mac artifacts. | Complete native launch, lifecycle, audio/controller and save qualification on a supported platform; do not infer Windows support from inherited build files. |
...
| re:Blue | All three user-owned Blue Dragon ISOs are available, and checksum-qualified Windows v1.0.0 resolves. The 2026-09-03 direct-source review of `installer_wizard`, `disc_install`, and `install_registry` confirms an interactive three-disc GDFX installer, source checks based on expected marker names/sizes, executable copying/restart, and a shared install registration (`HKCU\Software\Zolaware\reblue\Install` on Windows; XDG `reblue/install.toml` on Linux). No deterministic headless disc/root interface or exact source digest contract was found. The generic multi-disc and pinned-runtime capabilities now exist, but they do not authorize owning another application's global installation registry or fabricating its install marker. | Upstream exposes a deterministic, isolated install/data-root contract and maintainer-verifiable disc identity. Then bind the existing multi-disc staging to that interface, preserve `profiles`, `game`, `mods`, and installer state through replacement, and complete lifecycle and hands-on qualification. |
| Gen2Recomped: Pokémon Gold, Silver, and Crystal | Three opt-in beta Windows entries are cataloged against the checksum-qualified direct v0.7.35 release. Portcove supplies the exact Gold, Silver, or Crystal selector with the registered ROM so the shared upstream package cannot fall back to Pokémon Red. The source scan found no matching owned ROM: Gold SHA-1 `d8b8a3600a465308c9953dfa04f0081c05bdcb94`, Silver `49b163f7e57702bc939d642a18f591de55d92dae`, or Crystal Rev 1/Rev 0 `f2f52230b536214ef7c9924f483392993e226cfb` / `f4cd194bdee0d04ca4eac29e09b8e4e9d818c133`. | Supply any matching `.gbc` ROM, then run real import, retained-cache relaunch, update/rollback, save collection and hands-on gameplay qualification for that game. Repeat independently for each entry before adding its Windows automation badge. |
| Final Fantasy VII: Recompiled | Resolved by the 2026-09-04 follow-up. All three RomM CHDs match the exact normalized Track 01 identities. Portcove materializes them as a verified immutable BIN/CUE runtime set, v0.1.6 completes install and full verification, and the Windows build boots through the Sony and Final Fantasy VII title sequences with persistent memory cards. | Automated Windows qualification is complete. Perform hands-on gameplay and save/load testing before recording manual qualification or Supported status. |
...
| NocturneRecomp title-update variant | The supplied 1,548,288-byte LIVE title update has SHA-256 `226420345845a2077ee362ad68e8d444afce549a5eeb7f8c43aebae232fb4b8c`, matches the supplied base package, and independently applies as upstream version `v0.0.2.4`. The checksum-qualified v1.4.5 TU Windows executable is therefore viable, but Portcove currently accepts one source artifact per port and must not silently ignore or globally install the title update. The vanilla Windows entry is cataloged separately from the exact base package. | Add a generic compound source-materialization contract that binds the exact base STFS package and matching LIVE title update, extracts both transactionally, checks the patched identity expected by the direct game, and preserves the two-source identity through relink/export/import. Then catalog and qualify the TU variant independently. |
| Zelda3 | The active direct `snesrev/zelda3` v0.3 release documents the exact US source SHA-256 `66871d66be19ad2c34c927d6b14cd8eb6fc3181965b6e517cb361f7316009cfb` and requires `zelda3.sfc` beside `zelda3_assets.dat`. Its sole 1,847,661-byte runnable ZIP has neither a GitHub digest nor a checksum sidecar. | Upstream publishes SHA-256 integrity for a runnable release. Then catalog its exact source and portable paths and complete lifecycle plus hands-on qualification. |
| Zelda: Link's Awakening DX HD | The active direct `ryancheung/Zelda-LA-DX-HD` v2.0.0 release publishes a checksum-qualified 15,669,009-byte `Content.rar`, but it is not a complete runnable package. Upstream requires the `Data` directory from the removed original v1.0.0 itch.io release plus `Content` from `source.7z`; the current README still links to a removed GitHub release location, and no repository license was detected. | A maintained direct upstream publishes a complete runnable asset or deterministic updater, an exact identity for the required v1.0.0 source package, and clear redistribution/license terms. Then define its source and persistent-data contract and qualify it. |
...
- Campaign progress: the guarded Windows script now qualifies predecessor-to-candidate replacement, checks the complete expected NSIS-bundled executable hash, requires clean exit, and proves that the library database and a labeled test marker survive replacement and uninstall. An actual same-version 0.1.0 build replacement passed. This does not establish a future version-number upgrade.
- Remaining boundary: choose certificate custody and CI secret handling, sign release artifacts, qualify a version-number upgrade, then observe interactive installer choices, SmartScreen/Gatekeeper behavior, shortcuts/PATH integration, icon appearance in the Windows taskbar/Start menu and supported macOS/Linux shells, and clean uninstall on each target operating system. Do not represent image-level icon inspection or the current unsigned automated Windows smoke test as signed production installer and OS-shell validation.
...
- Campaign tooling: `scripts/qualification-report.mjs` captures shared core evidence and a session checklist without changing qualification flags. Actual update, rollback, failed preparation, and library-move activity is available in the isolated Paper Mario and Mega Man X6 qualification libraries; the report distinguishes filesystem markers from real game saves.
...
## Reopened product decisions

The campaign re-evaluated these audit follow-ups after completing the four Phase 5 capabilities. They remain deliberate product boundaries, rather than unexamined queue positions:

| Capability | Disposition and concrete prerequisite |
|---|---|
| Background job queue | Intentionally retained for a separate product change. Durable activity and cancellation now exist, but activity rows are history and interruption recovery, not replayable job specifications. A queue needs explicit restart/retry policy, persisted reviewed source/release intent, ordering, ownership across CLI/desktop processes, and visible consent for unattended work. V1 continues explicit foreground management with recoverable operations; automatically replaying an old review would change its authorization contract. |
| Mod packages and browser | Explicitly postponed to post-V1 by the user on 2026-09-03, including Dinomod recommendations. The isolated Dinomod startup test adds no product feature. Existing upstream-owned mod folders are preserved as user data; Portcove has no package provenance, integrity, file ownership/conflict, script/executable policy, or game-update/rollback contract for independently installed mods. Implement that reviewed model before any browser or package installation. |
| Desktop self-updates | Intentionally retained until signing custody, release provenance, platform installers, staged replacement, and updater rollback are qualified. Signed catalog metadata is deliberately unable to replace the Portcove executable and does not supply those guarantees. |
