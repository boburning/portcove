...
const themeBlockPattern =
  /(:root|\[data-theme=(?:"light"|'light')\])\s*\{([\s\S]*?)\r?\n\}/g;
const tokenBlocks = [...css.matchAll(themeBlockPattern)];
const rootBlock = tokenBlocks.find(([, selector]) => selector === ":root");
const lightBlock = tokenBlocks.find(([, selector]) =>
  selector.includes("light"),
);

if (!rootBlock)
  throw new Error("Theme check failed: styles.css has no :root token block.");
if (!lightBlock)
  throw new Error(
    "Theme check failed: styles.css has no light theme token block.",
  );
...
for (const theme of themes) {
  for (const token of requiredAliases) {
    if (!theme.tokens.has(token))
      failures.push(
        `${theme.name} theme is missing required semantic alias ${token}`,
      );
  }
}
...
for (const token of requiredLightOverrides) {
  if (!lightOverrides.has(token))
    failures.push(`light theme must explicitly override ${token}`);
}
for (const token of lightOverrides.keys()) {
  if (token.startsWith("--n64-"))
    failures.push(
      `light theme redefines primitive ${token}; override semantic aliases instead`,
    );
}
...
for (const [token, expected] of Object.entries(expectedPrimitives)) {
  ...
  if (actual !== expected)
    failures.push(
      `${token} must remain ${expected}, found ${actual ?? "missing"}`,
    );
}

const componentCss = css.replace(themeBlockPattern, "");
const literalColor = componentCss.match(
  /#[0-9a-f]{3,8}\b|rgba?\s*\(|hsla?\s*\(/i,
);
if (literalColor)
  failures.push(
    `component CSS contains literal color ${literalColor[0]}; use a semantic alias`,
  );
...
function resolveColor(tokens, token) {
  ...
  while (value.startsWith("var(")) {
    ...
    if (visited.has(nextToken))
      throw new Error(`Circular theme token reference at ${nextToken}`);
...
const contrastPairs = [
  ...
  [
    "interactive text on elevated surface",
    "--color-interactive-text",
    "--color-bg-elevated",
    4.5,
  ],
  ...
  [
    "signature action hover label",
    "--color-text-on-dark",
    "--color-accent-hover",
    4.5,
  ],
  [
    "signature action active label",
    "--color-text-on-dark",
    "--color-accent-active",
    4.5,
  ],
  ...
  [
    "success text on subtle surface",
    "--color-success-text",
    "--color-success-subtle",
    4.5,
  ],
  [
    "warning text on subtle surface",
    "--color-warning-text",
    "--color-warning-subtle",
    4.5,
  ],
  [
    "danger text on subtle surface",
    "--color-danger-text",
    "--color-danger-subtle",
    4.5,
  ],
  ...
  [
    "control border on inset surface",
...
for (const theme of themes) {
  for (const [
    ...
    if (ratio + Number.EPSILON < minimum)
      failures.push(
        `${theme.name}: ${label} is ${ratio.toFixed(2)}:1; needs ${minimum}:1`,
      );
  }
}
...
if (failures.length > 0) {
  ...
  const summaries = themes.map((theme) => {
    const themeResults = results.filter(
      (result) => result.theme === theme.name,
    );
