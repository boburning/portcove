...
const released = (version, overrides = {}) => ({
  tag_name: `v${version}`,
  draft: false,
  prerelease: true,
...
const approved = (version, overrides = {}) => ({
  version,
...
test("late maintenance cannot replace a higher preview and drafts stay excluded", () => {
  const candidates = [
    released("0.1.9", { published_at: "2026-09-09T12:00:00Z" }),
...
test("Preview-to-Stable waits, equal versions do not reinstall and RC-to-final selects the new artifact", () => {
  ...
  const final = released("1.0.0", {
    prerelease: false,
    artifact_sha256: "b".repeat(64),
  });
  const options = {
    currentVersion: "1.0.0-rc.1",
    eligibility: { "v1.0.0": approved("1.0.0") },
  };
  assert.equal(selectApplicationRelease([rc], "stable", options), null);
  assert.equal(
    selectApplicationRelease([rc, final], "stable", options).artifact_sha256,
    final.artifact_sha256,
  );
  assert.equal(
    selectApplicationRelease([final], "stable", {
      ...options,
  ...
  assert.equal(
    selectApplicationRelease([released("0.3.0+build.2")], "preview", {
      currentVersion: "0.3.0+build.1",
    }),
    null,
  );
  assert.throws(
...
test("target mismatch, withdrawal and holds cannot manufacture an eligible update", () => {
  ...
  for (const evidence of [
    approved("1.0.0", { held: true }),
    approved("1.0.0", { withdrawn: true }),
  ]) {
    assert.equal(
      selectApplicationRelease([release], "stable", {
        eligibility: { "v1.0.0": evidence },
      }),
      null,
    );
  }
  assert.throws(
    () =>
      selectApplicationRelease([release], "stable", {
        eligibility: { "v1.0.0": approved("1.1.0") },
      }),
    /invalid release eligibility/,
  );
  assert.throws(
    () =>
      selectApplicationRelease([release], "stable", {
        eligibility: { "v1.0.0": approved("1.0.0", { held: "true" }) },
      }),
    /inconsistent release eligibility/,
  );
});

const commit = "a".repeat(40);
const classification = (base, change, overrides = {}) => ({
...
test("reviewed frozen classifications produce deterministic initial-development versions", () => {
  for (const [base, change, expected] of [
    ["0.1.0", "patch", "0.1.1"],
    ["0.1.0", "minor", "0.2.0"],
...
test("classification rejects stale review/history, silent compatibility breaks and implicit finalization", () => {
  assert.throws(
    () =>
      proposeApplicationVersion(
        classification("0.1.0", "patch", { reviewed_commit: "b".repeat(40) }),
        ["0.1.0"],
      ),
  ...
  assert.throws(
    () =>
      proposeApplicationVersion(
        classification("1.0.0", "minor", {
          compatibility: "breaking",
          migration_notes: "Migrate the documented API",
        }),
        ["1.0.0"],
      ),
    /cannot carry/,
  ...
  assert.equal(
    proposeApplicationVersion(
      classification("0.1.0", "minor", {
        compatibility: "breaking",
        migration_notes: "Preserve and migrate data",
      }),
      ["0.1.0"],
    ).version,
    "0.2.0",
  ...
  assert.throws(
    () =>
      proposeApplicationVersion(
        classification("1.1.0-rc.1", "prerelease", {
          compatibility: "breaking",
          migration_notes: "Migrate the API",
        }),
        ["1.0.0", "1.1.0-rc.1"],
      ),
...
test("review explicitly starts new prerelease trains and preserves compatibility rules", () => {
  assert.equal(
    proposeApplicationVersion(
      classification("0.3.0", "premajor", { prerelease_identifier: "rc" }),
      ["0.3.0"],
    ).version,
    "1.0.0-rc.1",
  );
  assert.equal(
    proposeApplicationVersion(
      classification("1.0.0", "premajor", {
        prerelease_identifier: "beta",
        compatibility: "breaking",
        migration_notes: "Migrate the public contract",
      }),
      ["1.0.0"],
    ).version,
    "2.0.0-beta.1",
