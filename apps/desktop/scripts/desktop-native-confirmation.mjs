...
export function nativeConfirmation({ application, driverPid, output, artifacts }) {
  return async (title, button, expectedText, name, filePath) => {
    ...
    if (filePath) {
      ...
      assert.ok(path.isAbsolute(filePath) && relative && !relative.startsWith("..") && !path.isAbsolute(relative), "Native picker input must be an owned output fixture");
    }
    const result = spawnCommand("pwsh", ["-NoProfile", "-File", fileURLToPath(new URL("./native-confirmation.ps1", import.meta.url)), "-DriverProcessId", String(driverPid), "-ApplicationPath", application, "-Title", title, "-ExpectedText", expectedText, "-Button", button, ...(filePath ? ["-FilePath", filePath] : [])], { encoding: "utf8", windowsHide: true, timeout: 15_000 });
