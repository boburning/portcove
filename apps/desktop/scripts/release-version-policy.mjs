...
export function classifyApplicationVersion(version, productionEligible = false) {
  ...
  if (typeof productionEligible !== "boolean") throw new Error("production eligibility must be explicit boolean evidence");
...
function publishedRelease(release) {
  ...
  if (typeof githubPrerelease !== "boolean") throw new Error("published release is missing its prerelease flag");
  const tag = field(release, "tagName", "tag_name");
  if (typeof tag !== "string" || !tag.startsWith("v")) throw new Error("published application release must have a version tag");
  const version = tag.slice(1);
  const parsed = exactVersion(version);
  const publishedAt = field(release, "publishedAt", "published_at");
  if (Number.isNaN(Date.parse(publishedAt ?? ""))) throw new Error(`published release has an invalid published timestamp: ${publishedAt ?? "missing"}`);
...
function eligibilityEvidence(candidate, evidence) {
  ...
  if (evidence && (evidence.version !== version || typeof evidence.preview_eligible !== "boolean" || typeof evidence.production_eligible !== "boolean")) {
    throw new Error(`invalid release eligibility evidence for ${tag}`);
  }
  if (evidence && (["held", "withdrawn"].some(key => evidence[key] !== undefined && typeof evidence[key] !== "boolean")
      || (evidence.production_eligible && !evidence.preview_eligible))) throw new Error(`inconsistent release eligibility evidence for ${tag}`);
  return evidence;
}

function eligibleChannels(candidate, evidence) {
  const { version, parsed, githubPrerelease } = candidate;
  const classification = classifyApplicationVersion(version, evidence?.production_eligible ?? false);
  ...
  const previewEligible = evidence ? evidence.preview_eligible : parsed.major === 0 || parsed.prerelease.length > 0;
...
function selectionRequest(releases, channel, options) {
  ...
  if (options.eligibility !== undefined && (!options.eligibility || typeof options.eligibility !== "object" || Array.isArray(options.eligibility))) throw new Error("eligibility must be a separately verified map");
...
function uniquePrecedence(versions) {
  ...
  for (let index = 1; index < sorted.length; index += 1) {
    if (semver.eq(sorted[index - 1], sorted[index])) throw new Error("ambiguous application releases contain duplicate version precedence (equal SemVer precedence)");
  }
}
...
export function selectApplicationRelease(releases, channel, options = {}) {
  ...
  const candidates = releases.map(publishedRelease).map(candidate => eligibleCandidate(candidate, channel, options)).filter(Boolean);
  ...
  if (!candidate || (options.currentVersion !== undefined && !semver.gt(candidate.version, options.currentVersion))) return null;
  return candidate.release;
}

function reviewedClassification(classification) {
  if (!/^[a-f0-9]{40}$/.test(classification?.source_commit ?? "")) throw new Error("classification requires an exact source commit");
...
function publishedHistory(classification, publishedVersions) {
  if (!Array.isArray(publishedVersions)) throw new Error("published version inventory must be complete and explicit");
  publishedVersions.forEach(exactVersion);
  uniquePrecedence(publishedVersions);
  if (!publishedVersions.includes(classification.base_version)) throw new Error("classification base is absent from published history");
  if (publishedVersions.some(version => semver.gt(version, classification.base_version))) throw new Error("classification base is behind published history");
}
...
function advanceVersion(base, change, identifier) {
  ...
  if (change === "prerelease") {
    if (!base.prerelease.length) throw new Error("prerelease progression requires an existing prerelease train");
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  if (process.argv.length !== 4 || !inputPath) throw new Error("usage: release-version-policy.mjs classify|select|propose INPUT.json");
  const input = JSON.parse(await readFile(inputPath, "utf8"));
  let output;
  if (operation === "classify") output = classifyApplicationVersion(input.version, input.production_eligible);
  else if (operation === "select") output = selectApplicationRelease(input.releases, input.channel, input.options);
  else if (operation === "propose") output = proposeApplicationVersion(input.classification, input.published_versions);
