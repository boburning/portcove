...
export function combineTransportSchemas(schemas, contract = "output") {
  ...
  function add(name, schema) {
    ...
    if (Object.hasOwn(definitions, name) && JSON.stringify(definitions[name]) !== JSON.stringify(value)) {
  ...
  for (const [key, schema] of Object.entries(schemas)) {
    ...
    const matching = Object.entries(definitions).find(([, definition]) => JSON.stringify(definition) === JSON.stringify(body(schema)));
    const name = matching?.[0] ?? `${prefix}${key.split("_").map(part => part[0].toUpperCase() + part.slice(1)).join("")}`;
    add(name, schema);
    properties[key] = { $ref: `#/$defs/${name}` };
  }
  return { type: "object", title: `Transport${prefix}s`, properties, required: Object.keys(properties), additionalProperties: false, $defs: definitions };
}
...
async function main() {
  ...
  for (const [name, expected] of [
    ...
    if (fs.readFileSync(target, "utf8").replaceAll("\r\n", "\n") !== expected) throw new Error(`Generated TypeScript transport declarations differ from the Rust schema snapshot: ${name}`);
  }
