...
async function fixture() {
  ...
  const classification = { source_commit: source, reviewed_commit: source, base_version: "0.1.0", change: "minor", compatibility: "compatible" };
...
test("prepares exact coordinated metadata and retries without changing working files or index", async () => {
  ...
  assert.equal(git("show", `${result.prepared_commit}:Cargo.toml`), files["Cargo.toml"].replace('version = "0.1.0"', 'version = "0.2.0"'));
  assert.match(git("show", `${result.prepared_commit}:Cargo.lock`), /name = "third-party"\nversion = "0.1.0"/);
  assert.equal(JSON.parse(git("show", `${result.prepared_commit}:apps/desktop/src-tauri/tauri.conf.json`)).version, "0.2.0");
  assert.equal(git("show", "-s", "--format=%P", result.prepared_commit).trim(), classification.source_commit);
  assert.equal(git("for-each-ref", "--format=%(refname)", "refs/portcove/").trim().split("\n").length, 2);
...
test("rejects changed intent, reused version, stale history and partial receipts", async () => {
  const { root, classification, git } = await fixture();
  const result = await prepareReleaseVersion(root, classification, ["0.1.0"]);
  await assert.rejects(prepareReleaseVersion(root, { ...classification, change: "patch" }, ["0.1.0"]), /already allocated/);
  await assert.rejects(prepareReleaseVersion(root, classification, ["0.1.0", "0.2.0"]), /behind published history/);
...
test("invalid source metadata cannot reserve a version or silently update a dependency", async () => {
  ...
  git("add", "Cargo.lock"); git("commit", "-qm", "invalid lock identity");
  const source = git("rev-parse", "HEAD").trim();
  await assert.rejects(prepareReleaseVersion(root, { ...classification, source_commit: source, reviewed_commit: source }, ["0.1.0"]), /ambiguous workspace lock identity/);
