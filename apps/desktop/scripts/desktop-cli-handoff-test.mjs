...
export async function cliHandoffScenario({ browser, invoke, scenario, output, artifacts, cli, command }) {
  await scenario("native-contextual-cli-handoff", async () => {
    const bootstrap = (await invoke("get_bootstrap_status")).value;
    assert.equal(await realpath(bootstrap.library_root), await realpath(path.join(output, "moved-library")));
    const context = await invoke("get_cli_command_context", { generation: bootstrap.generation });
    assert.equal(context.ok, true);
    assert.equal(await realpath(context.value.executable), await realpath(cli));
    const stale = await invoke("get_cli_command_context", { generation: bootstrap.generation - 1 });
    ...
    const open = async (portId, view) => {
      ...
      await click(By.xpath('//summary[starts-with(normalize-space(.), "Release, sources & maintenance")]'));
    };
    ...
    assert.deepEqual(args, ["--library", context.value.library_root, "exec", "zelda64-recomp", "--"]);
    ...
    await browser.executeScript('arguments[0].scrollIntoView({ block: "center" });', await browser.findElement(launch));
    ...
    await browser.wait(until.elementLocated(By.css('[aria-label="Copy command template"]')), 10_000);
    const setup = await browser.findElement(By.css('[aria-label="Set up from the command line"]')).getText();
    assert.ok(setup.includes("source-path") && setup.includes("Template"));
    assert.equal((await browser.findElements(By.css('[aria-label="Copy setup command"]'))).length, 0);
    const result = path.join(output, "cli-handoff-result.json");
    await writeFile(result, JSON.stringify({ context: context.value, shell, program, args, stale_generation_rejected: true, unresolved_setup_is_template: true }, null, 2), { flag: "wx" }); artifacts.push(result);
  });
}
