...
export async function readinessScenario({ browser, scenario, output, artifacts, command, open }) {
  await scenario("native-missing-readiness-recovery", async () => {
    ...
    const observations = { injection: "omit one owned port's readiness from actual get_statuses responses; core remains launchable", before };
    const controls = reviewControls(browser);
    try {
      await browser.executeAsyncScript((portId, done) => {
        ...
        window.fetch = async function(input, ...args) {
          ...
          if (!Array.isArray(statuses) || !statuses.some(status => status.port_id === portId && status.readiness?.launchable === true)) return response;
          window.__portcoveReadinessProbe.injected++;
          return new Response(JSON.stringify(statuses.map(status => status.port_id === portId ? { ...status, readiness: null } : status)), {
            status: response.status, statusText: response.statusText, headers: response.headers,
          });
        };
        native.invoke("plugin:event|emit", { event: "portcove://library-changed", payload: "readiness-probe" }).then(() => done({ ok: true }), error => done({ error }));
      }, port.id).then(result => assert.equal(result.ok, true, JSON.stringify(result)));
      await browser.wait(async () => (await browser.findElement(By.css(".detail-panel")).getText()).includes("Readiness unavailable"), 10_000);
      ...
      try {
        observations.probe = await browser.executeScript(() => {
          ...
          const result = { injected: probe.injected, restored: window.fetch === probe.original };
          delete window.__portcoveReadinessProbe;
          return result;
        });
        assert.equal(observations.probe.restored, true);
        assert.ok(observations.probe.injected > 0, "The native omission must actually reach the renderer");
      } finally {
    ...
    await browser.wait(until.elementLocated(By.css(".detail-panel .primary-actions button")), 10_000);
    await browser.wait(async () => (await browser.findElement(By.css(".detail-panel .primary-actions button")).getText()) === "Play now", 10_000);
    assert.equal(await browser.findElement(By.css(".detail-panel .primary-actions button")).isEnabled(), true);
