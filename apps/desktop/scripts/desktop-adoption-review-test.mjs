...
export async function adoptionReviewScenario({ browser, invoke, scenario, library, output, artifacts, command, tool, host, confirmNative }) {
  await scenario("native-reviewed-existing-install-copy", async () => {
    ...
    await writeFile(path.join(original, "owned-next-version.bin"), "new application version", { flag: "wx" });
    ...
    const open = async () => {
      ...
      await input.clear(); await input.sendKeys(original);
    ...
    for (const expected of [original, paths.user_data_root, previous.path, "Matching saved files are replaced", "No automatic safety backup", "cannot cancel"]) assert.ok(text.includes(expected), expected);
    await click(button("Keep original setup"));
    assert.equal(await readFile(current, "utf8"), "current settings");
    await open(); await click(button("Continue to copy confirmation"));
    await confirmNative("Confirm adoption", "__observe__", paths.user_data_root, "adoption-native-before-consent");
    assert.equal(await readFile(current, "utf8"), "current settings");
    assert.equal(command(["status", port.id]).active.id, previous.id);
    await confirmNative("Confirm adoption", "Cancel", paths.user_data_root, "adoption-native-cancelled");
    ...
    const reviewed = await invoke("preview_adoption", { path: original, portId: port.id, generation });
    assert.equal(reviewed.ok, true);
    const stale = await invoke("adopt_port", { path: original, portId: port.id, generation: generation + 1, planSha256: reviewed.value.plan_sha256 });
    ...
    await click(button("Continue to copy confirmation"));
    await confirmNative("Confirm adoption", "Copy into Portcove", paths.user_data_root, "adoption-native-confirmed");
    await browser.wait(async () => (await browser.findElements(dialog)).length === 0, 15_000);
    const adopted = command(["status", port.id]).active;
    assert.notEqual(adopted.id, previous.id);
    assert.equal(await realpath(path.dirname(adopted.path)), await realpath(reviewed.value.destination.output_location.effective_output_directory));
    assert.equal(await readFile(current, "utf8"), "incoming settings");
    assert.deepEqual(await Promise.all(preserved.map(item => fileIdentity(item.path))), preserved);
    assert.deepEqual(command(["source", "list"]), sources);
    assert.equal(command(["status", "opengoal-jak1"]).active.id, otherInstall.id);
    assert.deepEqual(command(["backup", "list", port.id]).backups.map(item => item.id), [backup.id]);
    const result = path.join(output, "adoption-review-result.json");
    await writeFile(result, JSON.stringify({ preserved_files: preserved, previous_install: previous.id, adopted_install: adopted, reviewed_destination: reviewed.value.destination, dismissal_and_native_cancel_preserved_data: true, changed_saved_data_rejected: true, stale_generation_rejected: true, copied_saved_file: await fileIdentity(current), preserved_other_install: otherInstall.id, preserved_sources: sources, preserved_backup: backup.id, evidence: "native owned-file copy review and saved-data merge; no gameplay or physical interruption claim" }, null, 2), { flag: "wx" }); artifacts.push(result);
  });
}
