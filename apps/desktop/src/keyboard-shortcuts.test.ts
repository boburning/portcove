...
describe("global keyboard shortcuts", () => {
  it("maps navigation, palette, and search shortcuts", () => {
    ...
    expect(keyboardShortcutAction({ key: "4", metaKey: true })).toBe(
      "settings",
    );
    expect(keyboardShortcutAction({ key: "k", ctrlKey: true })).toBe(
      "toggle-palette",
    );
    expect(keyboardShortcutAction({ key: "/" })).toBe("focus-search");
    expect(keyboardShortcutAction({ key: "Escape", paletteOpen: true })).toBe(
      "close-palette",
    );
  });
  ...
  it("names the host modifier without assuming Control on macOS", () => {
    ...
    for (const platform of ["Win32", "Linux x86_64", ""])
      expect(commandShortcut("K", platform)).toBe("Ctrl K");
  });

  it("keeps workspace shortcuts behind the top modal inactive", () => {
    for (const key of ["1", "2", "3", "4", "k"]) {
      expect(
        keyboardShortcutAction({ key, ctrlKey: true, modalOpen: true }),
      ).toBeUndefined();
    }
    expect(
      keyboardShortcutAction({ key: "/", modalOpen: true }),
    ).toBeUndefined();
    expect(
