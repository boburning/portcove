...
export function primaryCliCommand(
  ...
  const interpreter: CommandShell =
    context.platform === "windows-x86-64" ? "powershell" : "posix";
  const executable = context.executable ?? "<portcove-executable>";
  const missing: string[] = context.executable
    ? []
...
export function quoteCliArg(value: string, shell: CommandShell) {
  if (value.includes("\0"))
    throw new Error("Command arguments cannot contain a null character.");
