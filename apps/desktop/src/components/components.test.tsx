...
const installRecord = (overrides: Partial<InstallRecord> = {}): InstallRecord => ({
  ...
  artifact: { asset_name: "sample.zip", sha256: "b".repeat(64), size: 1 }, manifest_sha256: "c".repeat(64), selected_executable: "sample.exe", runtime: null,
  ...overrides,
});
...
describe("desktop components", () => {
  ...
  it("shows ongoing parent progress after a child finishes while retaining failure notice", () => {
    ...
    const child: OperationEvent = { schema_version: 2, target: null, operation_id: "child", parent_operation_id: "parent", sequence: 3, timestamp_ms: 2,
  ...
  it("labels a new non-streaming task without reusing completed update progress", () => {
    const finished: OperationEvent = { schema_version: 2, parent_operation_id: null, target: null, operation_id: "old-update", sequence: 4, timestamp_ms: 1,
      operation: "update", type: "finished", result: "succeeded" };
  ...
  it("keeps verified backups usable while exposing degraded and recovery details", () => {
    ...
    const html = renderToStaticMarkup(<BackupHistory backups={backups} state="recovery_required" problems={[{
      kind: "recovery_required", backup_id: null, operation_id: "operation-1", path: "backups/sample/.deleting-operation-1",
  ...
  it("renders navigation, headers, status, and settings content", () => {
    const html = [
      ...
      renderToStaticMarkup(<StatusLayer error="Problem" clearError={vi.fn()} operation={{ schema_version: 2, parent_operation_id: null, target: null, operation_id: "install-1", sequence: 1, timestamp_ms: 1, operation: "install", type: "progress", phase: "install", completed: 1, total: 2 }} busy="install" />),
  ...
  it("shows the core host-readiness report with explicit tool states", () => {
    const html = renderToStaticMarkup(<SettingsView doctor={{
      ...
      host_tools: [{
        ...
        id: "future_tool", display_name: "Future tool", state: "missing", path: null, source: null, configuration_variable: "PORTCOVE_FUTURE_TOOL", purpose: "future source conversion", official_url: "https://example.com/tool",
      }],
    }} />);
  ...
  it("keeps recovery controls available for rejected saved sign-ins and explains environment overrides", () => {
    for (const source of ["credential_store", "environment"] as const) {
      const html = renderToStaticMarkup(<SettingsView github={{
        status: { source, authenticated: false, login: null, rate_limit: null, device_login_available: true },
  ...
  it("surfaces missing installed-library source requirements in settings", () => {
    const html = renderToStaticMarkup(<SettingsView libraryRoot="C:/Portcove" sourceNeeds={[{
      profile: { ...sourceProfile(), id: "sample-set", label: "Sample source set", kind: "file-set", accepted_extensions: [] },
  ...
  it.each([null, installRecord(), installRecord({ verified: false })])("does not infer blanket assurances from an install record", active => {
    const html = renderToStaticMarkup(<DetailPanel port={port} sourcePath="" setSourcePath={vi.fn()} actions={actions}
      status={{ ...portStatus(), port_id: port.id, channel: "stable", update_policy: "notify", active }} />);
  ...
  it("reports a recorded previous version without promising rollback or save compatibility", () => {
    const html = renderToStaticMarkup(<DetailPanel port={port} sourcePath="" setSourcePath={vi.fn()} actions={actions}
      status={{ ...portStatus(), port_id: port.id, channel: "stable", update_policy: "notify", active: installRecord(), previous: installRecord({ version: "0.9" }) }} />);
  ...
  it("does not turn a selected override into registered-source readiness", () => {
    ...
    const html = renderToStaticMarkup(<DetailPanel port={port} source={source} sourcePath="new.z64" setSourcePath={vi.fn()} actions={actions}
      status={{ ...portStatus(), port_id: port.id, channel: "stable", update_policy: "notify", active: installRecord(), readiness: { source: "current", launchable: true, blockers: [], pending_setup: false } }} />);
  ...
  it("explains the folder contract for a multi-disc source", () => {
    const html = renderToStaticMarkup(<DetailPanel port={port} sourceProfile={{
      ...sourceProfile(),      id: "sample-rom", label: "Three-disc set", kind: "psx-disc", accepted_extensions: ["chd"],
  ...
  it("renders an independently selectable required BIOS", () => {
    const html = renderToStaticMarkup(<DetailPanel port={{ ...port, bios_source_profile: "psx-bios" }}
      ...
      biosProfile={{ ...sourceProfile(), id: "psx-bios", label: "PlayStation SCPH-1001 BIOS", accepted_extensions: ["bin"] }} actions={actions} />);
  ...
  it("offers activation when an update is staged", () => {
    ...
    const status: PortStatus = { ...portStatus(), port_id: port.id, channel: "stable", update_policy: "stage", active: install, staged: { ...install, id: "2", version: "2.0", staged: true } };
  ...
  it("reveals eligible card targets only during native file drag and keeps a keyboard check in details", () => {
    ...
    const details = renderToStaticMarkup(<DetailPanel port={port} sourceProfile={{ ...sourceProfile(), id: "sample-rom", label: "Sample cartridge", kind: "file", accepted_extensions: ["z64"], accepted_sha1: [], accepted_sha256: [], disc: null, members: [] }} sourcePath="" setSourcePath={vi.fn()} inspectSource={vi.fn()} actions={actions} />);
  ...
  it("summarizes an installed library around play readiness", () => {
    ...
    const status: PortStatus = {
      ...
      last_update_check: {
        checked_at: 2,
        check: {
          port_id: port.id, channel: "stable", installed_version: "1.0", installed_runtime: null, required_runtime: null, installed_artifact: install.artifact, update_available: true,
