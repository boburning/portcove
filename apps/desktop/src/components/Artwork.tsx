...
export function ArtworkImage({
  ...
  port: Pick<PortDefinition, "id" | "name">;
  slot?: ArtworkSlot;
  className?: string;
}) {
  ...
  return (
    <section className="artwork-slot" aria-label={label} aria-busy={pending}>
      ...
      <details className="artwork-source">
        ...
        <ArtworkSource state={display.state} />
      </details>
    </section>
  );
}
...
function ArtworkSource({ state }: { state?: ArtworkState }) {
  ...
  if (!asset)
    return (
      <p>
        {state
          ? "Default fallback. No local image is selected."
          : "Artwork information is unavailable. Refresh to try again."}
      </p>
    );
  return (
    <dl>
