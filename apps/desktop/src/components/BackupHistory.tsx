...
export function BackupHistory({ backups, problems = [], state = "healthy", busy, generation = 0, restore, remove }: {
  ...
  const [selection, setSelection] = useState<{ backup: BackupRecord; action: "restore" | "delete" }>();
  ...
  return <div className="backup-history">
    ...
    {visible.map(backup => <div className="backup-row" key={backup.id} data-backup-id={backup.id}>
      ...
      <span className="backup-actions"><button data-focusable className="button-with-icon" disabled={Boolean(busy)} onClick={() => setSelection({ backup, action: "restore" })}><Icon glyph={RotateCcw} />Restore</button><button data-focusable className="danger icon-button" aria-label={`Delete backup from ${new Date(backup.created_at * 1000).toLocaleString()}`} disabled={Boolean(busy)} onClick={() => setSelection({ backup, action: "delete" })}><Icon glyph={Trash2} /></button></span>
    </div>)}
    {selection && <BackupReviewDialog key={`${selection.backup.id}:${selection.action}:${generation}`} backup={selection.backup} action={selection.action} generation={generation} apply={selection.action === "restore" ? restore : remove} close={() => setSelection(undefined)} />}
