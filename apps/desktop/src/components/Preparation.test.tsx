...
const plan: PreparationPlan = {
  format_version: 1, port_id: "sample", plan_sha256: "reviewed-plan",
  copy: { directories: [], files: [], skipped_entries: [], total_bytes: 1024 },
  inputs: {
    definition_sha256: "definition", host: "windows-x86-64", options: { target: "windows-x86-64", mode: "default" },
...
it("does not carry a late review across a library or port switch", async () => {
  ...
  vi.spyOn(desktopApi, "planPreparation").mockImplementation(() => new Promise(resolve => { complete = resolve; }));
  const run = vi.fn();
  await act(async () => root.render(<PreparationControl key="old" portId="sample" generation={7} disabled={false} run={run} />));
  await click("Review game preparation");
  await act(async () => root.render(<PreparationControl key="new" portId="other" generation={8} disabled={false} run={run} />));
