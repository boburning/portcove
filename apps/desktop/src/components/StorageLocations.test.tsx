...
describe("Storage locations", () => {
  it("keeps picker cancellation neutral and reviews a whole-library switch before applying it", async () => {
    ...
    await act(async () => {
      root.render(<LibrarySelectionCard selection={{ root: "E:/Portcove", source: "saved" }} choose={choose} switchLibrary={switchLibrary} reset={reset} />);
    });
  ...
  it("uses a separately named review before selecting the platform default", async () => {
    const reset = vi.fn().mockResolvedValue(undefined);
    await act(async () => {
      root.render(<LibrarySelectionCard selection={{ root: "E:/Portcove", source: "saved" }} choose={vi.fn()} switchLibrary={vi.fn()} reset={reset} />);
    });
