...
function report(stateCode = "recognized_exact"): SourceInspectionReport {
  ...
  return {
    ...
    profile_id: "game",
    health:
      stateCode === "source_missing"
        ? "missing"
        : stateCode === "source_changed"
          ? "changed"
          : "current",
    state_code: stateCode,
    summary:
      stateCode === "known_mismatch"
        ? "This file is a known mismatch."
        : "The selected source was checked locally.",
    next_action: "Review this result before setup.",
    registered: {
      profile_id: "game",
      path: "D:/Games/selected.zip",
      ...
      stateCode === "source_missing"
        ...
        : {
            ...
            components: [
              {
                id: "program",
                kind: "file_set_member",
                name: "program.bin",
                digests: [
                  {
                    algorithm: "sha256",
                    scope: "file-set-member",
                    value: "d".repeat(64),
                    size: 1024,
                  },
                ],
              ...
              },
              {
                ...
                size: 2048,
                track_count: 12,
                volume_id: "GAME_DISC_1",
              },
            ],
            assessment: {
              health: "current",
              classification,
              contract: { state: "supported", contract_id: "game-port" },
              admission,
              evidence: [],
            },
    ...
    problem:
      stateCode === "source_missing"
        ? { code: "not_found", message: "The registered file is missing." }
        : undefined,
    expected_identity: {
      id: "game",
      label: "Sample Game File",
      kind: "compound",
      aliases: [],
      tombstones: [],
      evidence_gap: null,
      variants: [
        {
          id: "edition",
          title: "Reviewed Edition",
          region: "USA",
          revision: "1.0",
          product_codes: ["GAME-01"],
          evidence_ids: ["identity-proof"],
          representations: [
            {
              id: "raw",
              kind: "compound",
    ...
    applications: [
      {
        ...
        contract: {
          ...
          supported_variant_ids: ["edition"],
          validator_contract_id: null,
          evidence_ids: ["identity-proof"],
          authority_ref: "Sample upstream requirement",
        ...
        contract_result:
          stateCode === "recognized_not_listed"
            ? { state: "recognized_not_listed", contract_id: "game-port" }
            : stateCode === "known_mismatch"
              ? { state: "known_incompatible", contract_id: "game-port" }
    ...
    evidence: [
      {
        ...
        authority_ref: "catalog row",
        reviewed_at: "2026-09-06",
        claim: "Expected source identity",
        immutable_url: "https://example.com/review/commit",
      },
    ],
    legacy: {
...
describe("source identity presentation", () => {
  it.each(["future_check_result", "constructor", "__proto__", "toString"])(
    "renders unknown result %s without claiming a match",
    (state) => {
      const value = report(state);
      value.applications[0].contract_result.state =
  ...
  it("does not treat unknown digest algorithms as a match or call inherited properties", () => {
    ...
    for (const component of value.inspection!.components) {
      for (const digest of component.digests)
        digest.algorithm = "constructor" as typeof digest.algorithm;
    }
    const html = renderToStaticMarkup(<SourceIdentityPanel report={value} />);
...
describe("source identity controls", () => {
  ...
  it("keeps copy and evidence controls named, keyboard reachable, and focused across a current report refresh", async () => {
    ...
    const copy = host.querySelector<HTMLButtonElement>(
      "[aria-label='Copy Calculated SHA256 for normalized content']",
    )!;
    const evidence = host.querySelector<HTMLButtonElement>(
      "[aria-label='Open reviewed evidence from Sample authority']",
    )!;
