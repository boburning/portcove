...
export function GameUpdateControl({ portId, generation, policy, busy, perform }: {
  ...
  return <section aria-label="Review game update">
    ...
    {plan && <GameUpdateReview plan={plan} disabled={busy || pending || !perform} pending={pending} confirm={confirm} review={() => { void review(); }} apply={() => { void apply(); }} />}
...
function GameUpdateReview({ plan, disabled, pending, confirm, review, apply }: {
  plan: GameUpdatePlan; disabled: boolean; pending: boolean; confirm: RefObject<HTMLButtonElement | null>; review: () => void; apply: () => void;
}) {
  const label = gameUpdateActionLabel(plan);
  if (!installPlanActionLabel(plan.plan.action)) return <div className="install-plan">
    <p role="alert">This version of Portcove cannot display this update plan. Review it again, or update Portcove if this continues.</p>
    <button ref={confirm} data-focusable disabled={disabled} onClick={review}>Review game update again</button>
  </div>;
  return <div className="install-plan">
    ...
    {label && <p>{plan.activate ? "The verified update becomes active and the current version remains available for rollback." : "The update is staged for later. Your active version stays unchanged."} Saved update settings are unchanged.</p>}
    {!label ? <p role="status">{plan.plan.action === "already_active" ? "This verified release is already active." : "An unverified local copy blocks this update. Verify or repair it first."}</p> : <button ref={confirm} data-focusable className="primary" disabled={disabled} onClick={apply}>{pending ? "Updating…" : label}</button>}
  </div>;
}
