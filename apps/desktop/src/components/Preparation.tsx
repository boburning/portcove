...
export type RunPreparation = (expectedPlan: string, onEvent: (event: OperationEvent) => void) => Promise<InstallRecord | undefined>;

export function PreparationControl({ portId, generation, disabled, run }: {
  portId: string; generation: number; disabled: boolean; run?: RunPreparation;
}) {
  ...
  useEffect(() => { if (plan) applyButton.current?.focus(); }, [plan]);

  const review = async () => {
    const current = ++request.current;
    setPending("review"); setError(undefined); setPlan(undefined); setMessage(undefined);
    try {
  ...
  const prepare = async () => {
    if (!plan || !run) return;
    const current = ++request.current;
    setPending("prepare"); setError(undefined); setMessage("Starting preparation…");
    try {
      ...
      if (request.current === current) { setPending(undefined); setOperationId(undefined); setPlan(undefined); }
    }
  };
  return <section aria-label="Prepare game data" className="preparation-control">
    <p>Prepare and verify the original game data before playing. Your current installation and saves are preserved.</p>
    {!plan && <button data-focusable className="primary wide" disabled={disabled || Boolean(pending) || !run} onClick={() => { void review(); }}>{pending === "review" ? "Checking preparation inputs…" : "Review game preparation"}</button>}
    {plan && <>
      <p><strong>Default setup · {plan.inputs.install.version}</strong></p>
