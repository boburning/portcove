...
it("shows the original path, installed impact and all dependents without removing anything", async () => {
  ...
  const remove = vi.spyOn(desktopApi, "removeSource"); const close = vi.fn();
  await act(async () => root.render(<SourceRemovalDialog profileId="source" generation={3} ports={ports} close={close} onRemoved={vi.fn()} />));
  expect(read).toHaveBeenCalledExactlyOnceWith("source", 3);
  for (const text of [preview.source.path, "Installed game", "Another compatible game", "Only this library's reference", "never schedules deletion", "no one-click undo"]) expect(container.textContent).toContain(text);
  expect(container.querySelector('[data-autofocus]')?.textContent).toBe("Keep source reference");
  await click("Keep source reference"); expect(close).toHaveBeenCalledOnce(); expect(remove).not.toHaveBeenCalled();
});
...
it("binds application to the review and does not treat native cancellation as removal", async () => {
  ...
  await act(async () => root.render(<SourceRemovalDialog profileId="source" generation={3} ports={ports} close={close} onRemoved={removed} />));
  ...
  expect(close).toHaveBeenCalledOnce(); expect(removed).not.toHaveBeenCalled(); expect(container.querySelector('[role="alert"]')).toBeNull();
});

it("blocks duplicate submissions within one event batch and requires fresh intent after rejection", async () => {
  const read = vi.spyOn(desktopApi, "previewSourceRemoval").mockResolvedValue(preview);
  let reject!: (value: Error) => void;
  const remove = vi.spyOn(desktopApi, "removeSource").mockImplementationOnce(() => new Promise((_resolve, no) => { reject = no; })).mockResolvedValue(preview);
  const close = vi.fn(); const removed = vi.fn().mockResolvedValue(undefined);
  await act(async () => root.render(<SourceRemovalDialog profileId="source" generation={3} ports={ports} close={close} onRemoved={removed} />));
  await act(async () => { const submit = button("Continue to removal confirmation"); submit.click(); submit.click(); });
  await click("Keep source reference"); expect(remove).toHaveBeenCalledOnce(); expect(close).not.toHaveBeenCalled();
  await act(async () => reject(new Error("Source or dependents changed")));
  expect(container.querySelector('[role="alert"]')?.textContent).toContain("Source or dependents changed");
  expect(container.textContent).not.toContain("Continue to removal confirmation");
  await click("Review source removal again"); expect(read).toHaveBeenCalledTimes(2);
  await click("Continue to removal confirmation"); expect(removed).toHaveBeenCalledOnce(); expect(close).toHaveBeenCalledOnce();
});

it("ignores a late preview from a previous source or library", async () => {
  let finish!: (value: SourceRemovalPreview) => void;
  vi.spyOn(desktopApi, "previewSourceRemoval").mockImplementationOnce(() => new Promise(resolve => { finish = resolve; })).mockResolvedValue({ ...preview, source: { ...preview.source, profile_id: "new", path: "new-original/game.bin" }, installed_dependent_port_ids: [] });
  const close = vi.fn(); const removed = vi.fn();
  await act(async () => root.render(<SourceRemovalDialog profileId="source" generation={3} ports={ports} close={close} onRemoved={removed} />));
  await act(async () => root.render(<SourceRemovalDialog profileId="new" generation={4} ports={ports} close={close} onRemoved={removed} />));
