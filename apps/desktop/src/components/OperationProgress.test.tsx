...
const event = (completed: number, total: number | null, phase = "download"): OperationEvent => ({
  schema_version: 2, operation_id: "owned-download", parent_operation_id: null, target: null,
  sequence: completed + 1, timestamp_ms: 1, operation: "install", type: "progress", phase, completed, total,
});
async function render(operation: OperationEvent) {
  await act(async () => root.render(<StatusLayer clearError={() => {}} busy="install" operation={operation} />));
}
beforeEach(() => {
...
describe("accessible operation progress", () => {
  ...
  it.each([
    ...
    { completed: 1_000_000, total: 2_000_000, now: "1000000", text: `${(1_000_000).toLocaleString()} of ${(2_000_000).toLocaleString()}` },
    { completed: 3, total: null, now: null, text: "Total not yet known" },
    { completed: -1, total: 10, now: null, text: "Total not yet known" },
    { completed: Number.NaN, total: 10, now: null, text: "Total not yet known" },
    { completed: 1, total: Number.POSITIVE_INFINITY, now: null, text: "Total not yet known" },
    { completed: Number.MAX_SAFE_INTEGER + 1, total: 10, now: null, text: "Total not yet known" },
  ])("keeps accessible counts bounded for $completed of $total", async ({ completed, total, now, text }) => {
