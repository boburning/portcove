...
describe("accessible operation progress", () => {
  it("updates visible counts without mutating the phase live region", async () => {
    ...
    try {
      for (const completed of [1, 20, 99, 100])
        await render(event(completed, 100));
      expect(changes).toHaveLength(0);
      expect(container.textContent).toContain("100 of 100");
      expect(status.textContent).toBe("Downloading files");
      expect(
        container.querySelector('[role="progressbar"]')?.closest("[aria-live]"),
      ).toBeNull();
  ...
  it.each([
    "future_internal_operation_code",
    "constructor",
    "__proto__",
    "toString",
  ])("safely handles unknown phase %s", async (phase) => {
