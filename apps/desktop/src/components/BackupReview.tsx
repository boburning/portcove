...
export function BackupReviewDialog({
  ...
  const {
    ...
    load: () =>
      desktopApi.previewBackupAction(
        backup.port_id,
        backup.id,
        action,
        generation,
    ...
    apply: (review) =>
      apply(review.preview.backup, review.preview.preview_sha256),
    close,
    failureMessage:
  ...
  return (
    <div className="scrim">
      ...
      >
        ...
        {pending === "review" && (
          <p role="status">Checking the backup and current saved data…</p>
        )}
...
function BackupReviewDetails({ review }: { review: BackupReview }) {
  ...
  return (
    <section
      className="backup-review-details"
      aria-label="Backup changes and preserved data"
    >
      <p>
        <strong>
          {backup.port_id} ·{" "}
      ...
      {action === "restore" ? (
        <>
          <p>
            The saved-data folder will contain this snapshot's data. The
            selected backup, other backups and installed game versions are
            preserved.
          </p>
          <p>
          ...
          <p>
            The game must be stopped. If its data changes after this review,
            review again before restoring.
          </p>
          <p>
            If interrupted, Portcove retains recovery data and checks the
            restore when the library reopens. Review any recovery notice before
            another restore.
          </p>
        </>
      ) : (
        <>
          <p>
            Current saved data, other backups and installed game versions are
            preserved.
          </p>
          <p>
            This snapshot cannot be recovered after deletion. No safety copy of
            the deleted snapshot is created.
          </p>
          <p>
            If interrupted, Portcove retains a deletion journal and reconciles
            the selected snapshot when the library reopens. An interrupted
            deletion may finish; it is not a reversible cancellation.
          </p>
        </>
      )}
    </section>
