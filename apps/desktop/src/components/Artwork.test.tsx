...
async function render(portId = "sample", generation = 7) {
  const port = { ...portDefinition(), id: portId, name: "An unusually long game title that stays separate from its cover" };
  await act(async () => root.render(<ArtworkProvider generation={generation}>
    <h2>{port.name}</h2><ArtworkImage port={port} /><ArtworkControls key={`${portId}:${generation}`} port={port} />
  </ArtworkProvider>));
}
...
function button(slot: "cover" | "detail", label: string) {
  const section = container.querySelector(`[aria-label="${slot === "cover" ? "Cover" : "Detail"} image"]`)!;
...
describe("local artwork controls", () => {
  it("selects and resets each slot through core and exposes local source information", async () => {
    ...
    const change = vi.spyOn(desktopApi, "importArtwork").mockImplementation(async (port, slot, _path, revision) => artworkState(port, slot, revision + 1, true));
    const reset = vi.spyOn(desktopApi, "resetArtwork").mockImplementation(async (port, slot, revision) => artworkState(port, slot, revision + 1));
  ...
  it("cancels the native picker without changing the choice and restores focus", async () => {
    ...
    await render(); await open(); await click("cover", "Choose local image");
    expect(change).not.toHaveBeenCalled();
    expect(button("cover", "Choose local image").disabled).toBe(false);
    expect(document.activeElement).toBe(button("cover", "Choose local image"));
  });

  it.each(["port", "library", "close"])("discards a picker result after a %s change", async kind => {
    let finish!: (path: string) => void;
    vi.spyOn(picker, "pickArtworkPath").mockReturnValue(new Promise(resolve => { finish = resolve; }));
    const change = vi.spyOn(desktopApi, "importArtwork");
    await render(); await open(); await click("cover", "Choose local image");
    if (kind === "close") {
      await act(async () => { const details = container.querySelector<HTMLDetailsElement>(".artwork-controls")!; details.open = false; details.dispatchEvent(new Event("toggle")); });
  ...
  it("keeps an unavailable preference visible and lets core refresh after a rejected change", async () => {
    vi.mocked(desktopApi.artwork).mockImplementation(async (port, slot) => ({ ...artworkState(port, slot, 5, true), availability: "unavailable", reason: "Selected file missing. Choice retained." }));
    vi.spyOn(picker, "pickArtworkPath").mockResolvedValue("E:/unsupported.svg");
    vi.spyOn(desktopApi, "importArtwork").mockRejectedValue(new Error("Static PNG or JPEG is required."));
    await render(); await open(); await click("cover", "Choose local image");
