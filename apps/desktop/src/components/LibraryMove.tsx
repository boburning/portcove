...
export function LibraryCopySummary({ plan, source, label }: { plan: Pick<LibraryMovePlan, "content" | "metadata" | "destination_root" | "required_bytes" | "available_bytes">; source: string; label: string }) {
  return <section className="adoption-plan adoption-review" aria-label={label}>
    ...
    {plan.metadata.application_versions.length > 0 ? <>
      <p>Active, previous, and staged versions keep their identities. These installation paths are relative to the library folder.</p>
      <ul>{plan.metadata.application_versions.map(install => <li key={install.id}><strong>{install.port_id}</strong> · {install.version}<br /><code>{install.path}</code></li>)}</ul>
    </> : <p>No installed versions are recorded in this plan.</p>}
    <h3>Copied folders and files</h3>
    <p>Open a folder to inspect the recorded paths. Each folder is copied from the source location to the same relative path at the destination.</p>
    {plan.content.map(tree => <LibraryCopyTree key={tree.relative_path} tree={tree} />)}
    <h3>Source registrations</h3>
    <p>{formatCountMessage(plan.metadata.source_references.length, { zero: "No source registrations are included.", one: "1 source registration keeps its recorded location.", other: "{count} source registrations keep their recorded locations.", unknown: "Source registration count is unavailable." })}</p>
    {plan.metadata.source_references.length > 0 && <details><summary data-focusable>Recorded source locations</summary><ul>
      {plan.metadata.source_references.map(reference => <li key={reference.profile_id}>{reference.profile_id}<br /><code>{reference.path}</code></li>)}
    </ul></details>}
    <p>Copying Source Inbox files does not redirect their registrations. Keep the recorded source locations available until you explicitly register another location.</p>
    <p>Original game files, saves, backups and artwork remain in place. Later saves and settings belong to the library you use; changes are not synchronized between the two locations. There is no single undo action that merges later changes back into the original.</p>
    <p>Once copying starts, this dialog cannot cancel it. If interrupted, use the recorded move or import recovery before another transfer. Keep both locations until the result is confirmed.</p>
  </section>;
}

function LibraryCopyTree({ tree }: { tree: LibraryMovePlan["content"][number] }) {
  const [expanded, setExpanded] = useState(false);
  const labels: Record<typeof tree.kind, string> = { application_versions: "Game versions", user_data: "Saved data", source_inbox: "Source Inbox", backups: "Backups", toolchains: "Preparation tools", local_artwork: "Local artwork" };
  return <details onToggle={event => setExpanded(event.currentTarget.open)}>
    <summary data-focusable>{Object.hasOwn(labels, tree.kind) ? labels[tree.kind] : "Other recorded content"} · {formatBytes(tree.copy.total_bytes)}</summary>
    {expanded && <>
      <p>Folder within both locations: <code>{tree.relative_path}</code></p>
      <p>{formatCountMessage(tree.copy.files.length, { zero: "No files are recorded in this folder.", one: "1 file will be copied.", other: "{count} files will be copied.", unknown: "File count is unavailable." })}</p>
      {tree.copy.files.length > 0 && <ul aria-label={`Files in ${tree.relative_path}`}>{tree.copy.files.map(file => <li key={file.relative_path}><code>{file.relative_path}</code> · {formatBytes(file.size)}</li>)}</ul>}
      {tree.copy.directories.length > 0 && <><p>Included subfolders, including empty folders:</p><ul aria-label={`Subfolders in ${tree.relative_path}`}>{tree.copy.directories.map(directory => <li key={directory}><code>{directory}</code></li>)}</ul></>}
    </>}
  </details>;
}
