...
function Workspace({ bootstrap, switchLibrary, resetLibrary }: { bootstrap: BootstrapStatus; switchLibrary: (path: string) => Promise<void>; resetLibrary: () => Promise<void> }) {
  ...
  const handleBack = useCallback(() => {
    ...
    else if (action === "close-adoption") { if (!adopting) ui.setAdoptOpen(false); }
    else if (action === "close-detail") ui.setSelectedId(undefined);
    else focusRegion("sidebar");
  }, [commandSurface.open, commandSurface.setOpen, ui.adoptOpen, ui.selectedId, ui.setAdoptOpen, ui.setSelectedId, adopting]);
...
function CurrentView({ data, ui, model, operations, github, updates, sourceHealth, appearance, bootstrap, switchLibrary, resetLibrary, nativeSourceDrag, hostToolActions }: {
  ...
  if (ui.view === "updates") return <UpdateCenter generation={bootstrap.generation} ports={data.catalog?.ports ?? []} statuses={model.statusMap} activities={data.activities} outcomes={updates.outcomes} busy={operations.busy} repair={data.doctor?.repair}
    checkAll={() => { void updates.checkAll(); }} onSelect={ui.setSelectedId} onOpenSources={() => ui.setView("settings")} />;
  if (ui.view === "settings") return <SettingsView generation={bootstrap.generation} ports={data.catalog?.ports ?? []} doctor={data.doctor} storage={data.storage} github={github} busy={operations.busy} sources={data.sources} appearance={appearance}
...
function AdoptionOverlay({ ui, operations, libraryGeneration }: { ui: UiState; operations: OperationState; libraryGeneration: number }) {
  ...
  return <AdoptionModal path={ui.adoptPath} setPath={ui.setAdoptPath} preview={planning.preview} copyFailed={planning.copyFailed} applying={planning.applying} busy={operations.busy} close={() => ui.setAdoptOpen(false)}
