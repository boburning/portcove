...
const source = (path: string): SourceRecord => ({ profile_id: "game", path, sha256: "a".repeat(64), size: 1, storage_sha256: "a".repeat(64), storage_size: 1, updated_at: 1 });
const report = (path: string): SourceInspectionReport => ({ schema_version: 1, profile_id: "game", health: "current", state_code: "recognized_exact", summary: path, next_action: "Continue", registered: source(path), applications: [], evidence: [], legacy: { registration_identity_not_recorded: false, variant_unspecified_records: [] } });

let root: Root;
let state: ReturnType<typeof useSourceHealth>;
function Fixture({ path, catalog, requested = true }: { path: string; catalog: string; requested?: boolean }) {
...
describe("source inspection intent", () => {
  it("does not hash a registration during ordinary browsing", async () => {
    ...
    await act(async () => root.render(createElement(Fixture, { path: "D:/Game.z64", catalog: "catalog", requested: false })));
  ...
  ])("does not let an old response replace %s", async (_label, oldPath, currentPath, oldCatalog, currentCatalog) => {
    const old = deferred<SourceInspectionReport>();
    const current = deferred<SourceInspectionReport>();
    vi.spyOn(desktopApi, "inspectSource").mockReturnValueOnce(old.promise).mockReturnValueOnce(current.promise);
    await render(oldPath, oldCatalog);
    await render(currentPath, currentCatalog);
    await act(async () => { current.resolve(report(currentPath)); await current.promise; });
    expect(state.inspections.get("game")?.summary).toBe(currentPath);
    await act(async () => { old.resolve(report(oldPath)); await old.promise; });
    expect(state.inspections.get("game")?.summary).toBe(currentPath);
  });
});
