...
function thumbnailUrl(thumbnail: ArtworkThumbnail, state: ArtworkState) {
  if (thumbnail.asset_sha256 !== state.choice.asset_sha256 || thumbnail.choice_revision !== state.choice.revision
    || thumbnail.png.length === 0 || thumbnail.png.length > 1024 * 1024) throw new Error("The artwork preview changed. Refresh to try again.");
  let binary = "";
  for (let offset = 0; offset < thumbnail.png.length; offset += 8192) binary += String.fromCharCode(...thumbnail.png.slice(offset, offset + 8192));
...
export class ArtworkCache {
  ...
  private key(portId: string, slot: ArtworkSlot) { return JSON.stringify([portId, slot]); }

  read(portId: string, slot: ArtworkSlot): ArtworkDisplay { return this.entries.get(this.key(portId, slot)) ?? empty; }
  ...
  private async display(portId: string, slot: ArtworkSlot, state: ArtworkState) {
    ...
    const image = state.availability === "available" && previous.state?.choice.asset_sha256 === state.choice.asset_sha256
      && previous.state.choice.revision === state.choice.revision ? previous.image : undefined;
    this.publish(portId, slot, { state, image, loading: state.availability === "available" });
    if (state.availability !== "available") return;
    try {
      const thumbnail = await desktopApi.artworkThumbnail(portId, slot, state.choice.revision, this.generation);
      this.publish(portId, slot, { state, image: thumbnailUrl(thumbnail, state), loading: false });
    } catch (error) {
      this.publish(portId, slot, { state, error: errorText(error), loading: false });
    }
  }
  ...
  change(portId: string, slot: ArtworkSlot, revision: number, path: string | null, stillCurrent: () => boolean) {
