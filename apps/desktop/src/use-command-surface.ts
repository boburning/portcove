...
export function useCommandSurface({ recent, installedCount, busy, setView, setAdoptOpen, setSelectedId, checkAll }: {
  ...
  const commands = useMemo<PaletteCommand[]>(() => [
    ...
    { id: "settings", label: "Open settings", description: "Manage sources, GitHub, storage, and appearance", icon: Settings, shortcut: commandShortcut("4"), keywords: "navigate preferences", action: () => setView("settings") },
