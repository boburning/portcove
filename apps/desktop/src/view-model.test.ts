...
const port = (
  ...
  summary: `${id} summary`,
  project_url: `https://example.com/${id}`,
  support_tier: channels[0],
  channels,
...
const installRecord = (
  ...
  verified: true,
  staged: false,
  artifact: { asset_name: "alpha.zip", sha256: "b".repeat(64), size: 1 },
  manifest_sha256: "c".repeat(64),
  selected_executable: "alpha.exe",
  runtime: null,
  ...overrides,
});
...
describe("catalog view model", () => {
  ...
  it("uses only update snapshots that still describe the active version and channel", () => {
    ...
    expect(
      currentUpdateSnapshot({
        ...status,
        active: { ...status.active!, version: "2.0" },
        last_update_check: snapshot,
      }),
    ).toBeUndefined();
    expect(
      currentUpdateSnapshot({
  ...
  it("groups missing source and BIOS requirements for installed ports only", () => {
    const configured = [
      ...
      {
        ...ports[1],
        source_profile: "beta-source",
        bios_source_profile: "shared-source",
      },
    ];
    const statuses = indexStatuses([
      status,
      {
        ...status,
        port_id: "beta",
        active: { ...status.active!, port_id: "beta" },
      },
    ]);
    const profiles = [
      {
