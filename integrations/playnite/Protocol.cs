...
namespace Portcove.ReferenceClient
{
    ...
    internal sealed class ProtocolStream
    {
        ...
        internal void Line(string line)
        {
            ...
            {
                if (Json.Number(record, "schema_version") != Schema || Json.Text(record, "command") != command)
                    throw new InvalidOperationException("Unsupported Portcove response. This client requires API schema 42; install a matching CLI/client pair.");
