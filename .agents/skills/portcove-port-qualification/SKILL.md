...
Establish direct upstream identity, exact runnable asset/platform, independent checksum authority, required source variant/representation, persistence paths, and fit with existing generic adapters. A local hash records bytes; it does not independently authenticate an upstream download. Missing optional gameplay evidence alone does not block unrelated promised operations.
...
For a hosted release selector, test whether one unchanged definition selects both ordinary release N and a controlled N+1, with an independently advertised digest for each. Pair that positive control with ambiguity, platform, integrity, and qualification-non-inheritance refusals. When upstream exposes only one or zero releases, label future-layout evidence synthetic or unavailable rather than claiming a real naming history.
...
Use an explicitly isolated library and authorized source files. Exercise promised operations through core/CLI: source validation, install, launch preflight, update, rollback and persistence where applicable. Never implement title-specific Rust behavior when catalog data or a bounded generic adapter capability can express it. Run `just test-rust -p portcove-core embedded_catalog_is_valid_and_contains_lighthouse` immediately after the first definition edit. Catalog admission follows [continuous admission](../../../docs/CATALOG.md#continuous-admission): a Cataloged entry may retain honest Unknown gameplay or optional evidence, while every operation, platform, support, and lifecycle claim needs matching scoped evidence. A schema pass, download, or launch alone is not lifecycle qualification. If the adapter lacks a required ownership class, identify the exact generic gap without misclassifying output.
