...
test("renders desktop first with readable labels and exact tag-bound links", () => {
  ...
  assert.match(section, /releases\/download\/v0\.1\.0-alpha\.2\/Portcove_0\.1\.0-alpha\.2_x64-setup\.exe/);
...
test("generation is idempotent and preserves reviewed prose and categorized changes", () => {
  const body = "# Portcove 0.1.0-alpha.2\n\nReviewed introduction.\n\n## Features\n\n- Kept change.\n\n## Known limitations\n\n- Kept limitation.\n";
...
test("regeneration removes only the marked section and preserves historical links", () => {
  const historical = "Historical Alpha 1: https://github.com/boburning/portcove/releases/tag/v0.1.0-alpha.1\n";
  const merged = mergeDownloadSection(`${renderDownloadSection(inventory)}\n\n${historical}`, inventory);
...
test("refuses malformed or duplicate generated sections instead of discarding reviewed prose", () => {
  ...
  const section = renderDownloadSection(inventory);
  assert.throws(() => mergeDownloadSection(`${section}\n\nReviewed.\n\n${section}`, inventory), /incomplete or duplicate/);
});
