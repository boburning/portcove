...
test("first attempts include initial queue time and reruns use their own start", () => {
  ...
  assert.equal(
    attempt({
      run_attempt: 2,
      run_started_at: stamp(3_600),
      updated_at: stamp(3_800),
    }).seconds,
    200,
  );
  assert.equal(
...
test("cancelled, failed and running attempts remain visible outside successful percentiles", () => {
  const summary = summarizeHistory([
    attempt(),
    attempt({ id: 2, conclusion: "cancelled" }),
    attempt({ id: 3, conclusion: "failure" }),
    attempt({ id: 4, status: "in_progress", conclusion: null }),
  ]);
...
test("rerun recovery retains the failed jobs and requires the same run and commit", () => {
  const failed = summarizeAttempt(run({ conclusion: "failure" }), [
    job({ name: "native Intel", conclusion: "timed_out" }),
  ]);
  const passed = attempt({ run_attempt: 2 });
  assert.deepEqual(summarizeHistory([failed, passed]).rerunRecoveries, [
    {
      runId: 1,
      sha: "a".repeat(40),
      failedAttempt: 1,
      passedAttempt: 2,
      failedJobs: ["native Intel"],
      url: failed.url,
    },
  ]);
...
test("percentiles separate first attempts from reruns and require twenty tail samples", () => {
  const first = Array.from({ length: 20 }, (_, index) =>
    attempt({ id: index + 1, updated_at: stamp(index + 1) }),
  );
  const summary = summarizeHistory([
    ...first,
    attempt({
  ...
  assert.deepEqual(summary.successfulFirstAttempts, {
    count: 20,
    p50Seconds: 10,
    p95Seconds: 19,
    maxSeconds: 20,
  ...
  const request = async (route, paginate) => {
    ...
    if (route.includes("/jobs?")) {
      ...
      return [
        ...
        { total_count: 2, jobs: [job({ name: "frontend" })] },
      ];
    }
    return run({
      run_attempt: number,
      conclusion: number === 1 ? "failure" : "success",
    });
  };
...
test("date cutoff keeps all attempts of eligible runs without mixing an older workflow", async () => {
  const request = async (route) => {
    if (route.includes("/workflows/"))
      return {
        workflow_runs: [
          { id: 1, run_attempt: 1, created_at: stamp(10) },
          { id: 2, run_attempt: 1, created_at: stamp(0) },
        ],
      };
