...
test("mixed protected unknown mode symlink and unusual path changes force full CI", () => {
  const cases = [
  ...
  for (const contents of cases) assert.equal(classifyChanges(parseRawDiff(contents)).mode, "full");
});
...
test("malformed incomplete or empty change discovery fails closed", () => {
  ...
  assert.equal(classifyChanges([]).mode, "full");
});
...
test("main and release events remain exhaustive without consulting a diff", () => {
...
test("GitHub outputs are complete single-line values", async (t) => {
  ...
  await writeGithubOutputs(output, {
    mode: "prose",
    reason: "reviewed-informational-prose-only",
  ...
  for (const key of ["mode", "reason", "files_json", "base", "merge_base", "head", "checkout"])
