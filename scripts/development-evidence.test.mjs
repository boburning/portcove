...
test("partial and failed evidence cannot become a pass", () => {
  ...
  assert.equal(evidenceOutcome([{ scenario: "a", outcome: "passed" }, { scenario: "b", outcome: "not-run" }]), "incomplete");
...
test("a vanished executable can retain its initial identity and failure evidence", async () => {
  ...
  try {
    ...
    await rm(executable);
    const report = await writeEvidence(directory, { revision: "a".repeat(40), executable,
      capturedExecutable, checks: [{ scenario: "executable-identity", outcome: "failed" }], method: "isolated-fixture" });
...
test("evidence hashes bytes and never replaces prior observations", async () => {
  ...
  try {
    const executable = path.join(directory, "fixture");
    await writeFile(executable, "abc");
    const input = { revision: "a".repeat(40), executable, checks: [{ scenario: "fixture", outcome: "passed" }], method: "isolated-fixture" };
    const report = await writeEvidence(directory, input);
    assert.equal(report.executable.sha256, "ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad");
