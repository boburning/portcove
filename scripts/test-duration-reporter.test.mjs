...
function event(duration, type = "test") {
  return {
    type: "test:pass",
    data: { name: "fixture", details: { type, duration_ms: duration } },
  };
}
...
test("individual slow tests are identified independently of suite duration", () => {
  ...
  assert.equal(
    isSlowTest({
      type: "test:pass",
      data: { ...event(5_001).data, skip: true },
    }),
    false,
  );
});
...
test("UI timing reports slow tests and rejects missing or incomplete measurements", () => {
  const report = (duration) => ({
    ...
    testResults: [
      {
        assertionResults: [{ status: "passed", fullName: "fixture", duration }],
      },
    ],
  });
  assert.deepEqual(checkVitestDurations(report(5_000)), {
    count: 1,
    longest: 5_000,
    slow: [],
  });
