...
export function evaluateFallowReport(report) {
  const failures = [];
  if (report.check.total_issues)
    failures.push(
      `${report.check.total_issues} dead-code or dependency findings`,
    );
  ...
  if (report.health.summary.severity_critical_count) {
    failures.push(
      `${report.health.summary.severity_critical_count} critical complexity findings`,
    );
  }
  return {
    failures,
...
async function main() {
  ...
  if (!reportPath)
    throw new Error("usage: node check-fallow-report.mjs <report.json>");
