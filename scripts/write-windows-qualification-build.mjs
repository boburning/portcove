...
const { values } = parseArgs({ options: {
  repository: { type: "string" }, installer: { type: "string" }, cli: { type: "string" },
  desktop: { type: "string" }, predecessor: { type: "string" },
  "predecessor-version": { type: "string" }, output: { type: "string" },
} });
for (const name of ["repository", "installer", "cli", "desktop", "predecessor", "predecessor-version", "output"]) {
...
if (status) throw new Error("Candidate checkout has tracked changes; build records require an exact clean checkout");
...
async function describe(input, name) {
  ...
  return { relative_path: item.relative, bytes: bytes.length, sha256: createHash("sha256").update(bytes).digest("hex") };
}
...
for (const relative of [
  ...
  execFileSync("git", ["-C", repository, "diff", "--quiet", commit, "--", relative], { windowsHide: true });
  tools[relative] = { relative_path: relative, bytes: disk.length, sha256: createHash("sha256").update(disk).digest("hex") };
}
...
process.stdout.write(`${JSON.stringify({ output, commit, sha256: createHash("sha256").update(body).digest("hex") })}\n`);
