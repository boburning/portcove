...
export function main() {
  ...
  try {
    ...
    process.stderr.write(result.stderr ?? "");
    throw new Error(
      `rscheck returned an invalid JSON report: ${error.message}`,
    );
  }
  ...
  for (const finding of assessment.warnings) {
    console.warn(
      `rscheck advisory [${finding.rule_id}] ${location(finding)}: ${finding.message}`,
    );
  }
  for (const finding of assessment.denies) {
    console.error(
      `rscheck failure [${finding.rule_id}] ${location(finding)}: ${finding.message}`,
    );
  }
  console.log(
