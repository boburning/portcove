...
function parseArguments(argv) {
  ...
  for (let index = 0; index < argv.length; index += 2) {
    ...
    if (
      ...
      throw new Error("invalid or duplicate release selection argument");
    options[key] = argv[index + 1];
  }
  if (!options.channel || !options.input)
    throw new Error(
      "usage: select-release-channel.mjs --channel preview|stable --input releases.json [--eligibility evidence.json] [--current-version VERSION] [--target TARGET]",
    );
  return options;
}
...
if (process.argv[1] && path.resolve(process.argv[1]) === scriptPath) {
  ...
  const selected = selectReleaseForChannel(
    JSON.parse(await readFile(options.input, "utf8")),
    options.channel,
    {
      eligibility: options.eligibility
        ? JSON.parse(await readFile(options.eligibility, "utf8"))
        : undefined,
