...
export function validateBrandManifestDefinition(manifest) {
  ...
  for (const [index, asset] of manifest.assets.entries()) {
    ...
    if (
      ...
      !allowedRoot ||
      path.posix.extname(normalizedPath) !== ".png"
    ) {
    ...
    if (!asset?.category || !asset?.role)
      errors.push(`brand manifest ${label} must name a category and role`);
    if (
  ...
  await Promise.all(
    manifest.assets.map(async (asset) => {
      ...
      if (hash !== asset.sha256) {
        errors.push(
          `brand asset ${asset.id} SHA-256 ${hash} does not match manifest ${asset.sha256}`,
        );
      }
      try {
        ...
        if (png.colorMode !== asset.color_mode) {
          errors.push(
            `brand asset ${asset.id} color mode ${png.colorMode} does not match manifest ${asset.color_mode}`,
          );
        }
      } catch (error) {
...
export function validateModelManifestDefinition(manifest) {
  ...
  for (const [name, expected] of Object.entries({
    ...
    if (geometry[name] !== expected)
      errors.push(`model manifest geometry ${name} must be ${expected}`);
  }
  if (
  ...
  for (const requiredId of requiredModelFileIds) {
    if (!ids.has(requiredId))
      errors.push(`model manifest is missing required file id: ${requiredId}`);
  }
  return errors;
}
...
async function collectModelManifest(root) {
  ...
  try {
    ...
    return {
      fileCount: 0,
      errors: [`model manifest could not be read: ${error.message}`],
    };
  }
  ...
  await Promise.all(
    manifest.files.map(async (file) => {
      try {
        ...
        if (hash !== file.sha256) {
          errors.push(
            `model file ${file.id} SHA-256 ${hash} does not match manifest ${file.sha256}`,
          );
        }
        if (buffer.length !== file.bytes) {
          errors.push(
            `model file ${file.id} byte size ${buffer.length} does not match manifest ${file.bytes}`,
          );
        }
      } catch (error) {
        errors.push(
...
const metadataRules = [
  ...
  (metadata) =>
    metadata.tauri.productName === "Portcove" &&
  ...
  (metadata) =>
    metadata.tauri.bundle?.active === true
      ? undefined
  ...
  (metadata) =>
    metadata.tauri.bundle?.licenseFile === "../../../LICENSE-MIT"
      ? undefined
...
export function validateReleaseMetadata(metadata, options = {}) {
  return [
    ...
    ...(metadata.missingFiles ?? []).map(
      (missingPath) => `required release file is missing: ${missingPath}`,
    ),
...
async function main() {
  ...
  console.log(
    `Release metadata check passed: Portcove ${metadata.cargo.version}${tagSuffix}; ${requiredProjectFiles.length} required files present; ${metadata.brandAssetCount} brand assets and ${metadata.modelFileCount} model files verified.`,
  );
}
