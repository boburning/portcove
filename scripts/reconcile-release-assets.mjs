...
async function walk(root) {
  ...
  for (const entry of await readdir(root, { withFileTypes: true })) {
    ...
    if (entry.isSymbolicLink()) throw new Error(`release matrix contains a symbolic link: ${entryPath}`);
...
export async function reconcileReleaseAssets(inputRoot, outputRoot, options = {}) {
  ...
  if (inventoryPath) await assertOwnedUnlinkedPath(projectRoot, inventoryPath, "release inventory output");
  ...
  for (const label of releaseLabels) {
    ...
    for (const file of files) {
      ...
      const supportedLayout = parts.length === 1
        || (parts.length === 2 && parts[0] === "release-upload");
      if (!supportedLayout) throw new Error(`${label} contains an unexpected staged path: ${path.relative(root, file)}`);
    }
    ...
    assertExactArtifactNames(expectedNames, entries.map(entry => entry.name), `${label} checksum entries`);
    assertExactArtifactNames([...expectedNames, manifestName], files.map(file => path.basename(file)), `${label} staged files`);
    for (const entry of entries) {
  ...
  await assertOwnedUnlinkedPath(projectRoot, output, "release aggregate output");
  for (const artifact of selected) await copyFile(artifact.file, path.join(output, artifact.filename));
  ...
  return {
    assets: selected.map(artifact => artifact.filename).concat(policy.checksum_manifest).sort(),
    inventory,
  };
}
...
function parseArguments(argv) {
  ...
  for (let index = 0; index < argv.length; index += 2) {
    ...
    if (!["--input", "--output", "--project-root", "--version", "--tag", "--inventory"].includes(name) || !value) {
      throw new Error("usage: reconcile-release-assets.mjs --input PATH --output PATH [--version VERSION] [--tag TAG] [--inventory PATH]");
    }
    if (["--input", "--output", "--project-root", "--inventory"].includes(name)) {
      const key = name === "--project-root" ? "projectRoot" : name === "--inventory" ? "inventoryPath" : name.slice(2);
...
if (process.argv[1] && path.resolve(process.argv[1]) === scriptPath) {
  ...
  console.log(`Reconciled ${result.assets.length} public release assets from all four platform jobs.`);
}
