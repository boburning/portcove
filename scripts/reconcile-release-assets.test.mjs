...
test("inventory binds labels, exact tag links, sizes, and checksums to validated packages", async t => {
  ...
  assert.equal(result.inventory.checksum_url, "https://github.com/boburning/portcove/releases/download/v0.1.0-alpha.2/SHA256SUMS.txt");
  ...
  assert.equal(windows.download_url, `https://github.com/boburning/portcove/releases/download/v0.1.0-alpha.2/${windows.filename}`);
...
test("rejects missing jobs and package outputs not declared by policy", async t => {
  ...
  await rm(path.join(missingJob.input, "release-build-linux-x86_64"), { recursive: true });
  await assert.rejects(reconcile(missingJob), /must contain exactly/);

  const unexpected = await fixture(t);
  await writeFile(
    path.join(unexpected.input, "release-build-windows-x86_64/release-upload/Portcove_0.1.0-alpha.2_x64.msi"),
    "not declared",
  );
...
test("rejects duplicate, unsafe, malformed, and mismatched checksum entries", async t => {
  ...
  const manifest = path.join(duplicate.input, "release-build-macos-aarch64/release-upload/SHA256SUMS-macos-aarch64.txt");
  ...
  const unsafeManifest = path.join(unsafe.input, "release-build-windows-x86_64/release-upload/SHA256SUMS-windows-x86_64.txt");
  ...
  const malformedManifest = path.join(malformed.input, "release-build-linux-x86_64/release-upload/SHA256SUMS-linux-x86_64.txt");
  ...
  const changedCli = path.join(changed.input, `release-build-windows-x86_64/release-upload/portcove-cli-${version}-windows-x86_64.zip`);
...
test("rejects output paths that could remove inputs or publish internal inventory", async t => {
  const paths = await fixture(t);
  await assert.rejects(
    reconcileReleaseAssets(paths.input, paths.root, { ...options, projectRoot: paths.root }),
    /unsafe or overlapping/,
  );
  await assert.rejects(
    reconcileReleaseAssets(paths.input, path.join(paths.input, "aggregate"), { ...options, projectRoot: paths.root }),
    /unsafe or overlapping/,
  );
  await assert.rejects(
    reconcile(paths, { inventoryPath: path.join(paths.output, "release-inventory.json") }),
...
test("rejects packages hidden in arbitrary nested staging paths", async t => {
  ...
  const nested = path.join(paths.input, "release-build-windows-x86_64", "release-upload", "nested", name);
...
test("refuses linked aggregate and inventory ancestry without touching the link targets", async t => {
  ...
  assert.equal(await readFile(path.join(outsideAggregate, "sentinel.txt"), "utf8"), "preserve aggregate");
  ...
  await assert.rejects(
    reconcile(inventory, { inventoryPath: path.join(inventoryLink, "release-inventory.json") }),
    /symbolic-link or reparse-point/,
  );
  assert.equal(await readFile(path.join(outsideInventory, "sentinel.txt"), "utf8"), "preserve inventory");
});
