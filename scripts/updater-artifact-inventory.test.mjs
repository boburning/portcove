...
import { artifactName, loadPackagePolicy, packagesForPlatform, releaseLabels } from "./release-package-policy.mjs";
import { stageUpdaterInventory, updaterIdentity, verifyUpdaterInventory } from "./updater-artifact-inventory.mjs";
...
async function fixture(t, label = "windows-x86_64") {
  ...
  for (const entry of packagesForPlatform(policy, label)) {
    const directory = entry.interface === "cli" ? "release-assets" : `target/release/bundle/${entry.format}`;
    await mkdir(path.join(root, directory), { recursive: true });
    await writeFile(path.join(root, directory, artifactName(entry, version)), `${entry.id} final bytes`);
  }
  ...
  let verified = 0;
  const options = {
    projectRoot: root, label, revision, publicKey,
    output: path.join(root, "staged"), input: path.join(root, "staged"),
...
test("missing signatures and failed verification never produce successful evidence", async t => {
  ...
  f.options.verifySignature = async () => { throw new Error("wrong signing key"); };
...
test("rejects duplicate packages, wrong target/version/revision and extra assets", async t => {
  ...
  const mutations = [
    ...
    data => { data.updater.target = "darwin-aarch64"; },
    data => { data.version = "0.2.0"; },
    data => { data.source_commit = "b".repeat(40); },
    data => { data.packages[0].filename = "../outside"; },
    data => { data.updater.signature.filename = "other.sig"; },
  ];
...
test("rejects overlapping staging paths and version override inheritance", async t => {
  const f = await fixture(t);
  await assert.rejects(stageUpdaterInventory({ ...f.options, output: path.join(f.root, "target/release/bundle/new") }), /overlap/);
  await stageUpdaterInventory(f.options);
  await assert.rejects(verifyUpdaterInventory({ ...f.options, version: "0.1.0" }), /identity mismatch/);
  await assert.rejects(verifyUpdaterInventory({ ...f.options, revision: "HEAD" }), /exact source commit/);
});

test("release-only overlay preserves ordinary secret-free builds and updater package baseline", async () => {
  const base = JSON.parse(await readFile(new URL("../apps/desktop/src-tauri/tauri.conf.json", import.meta.url)));
  const overlay = JSON.parse(await readFile(new URL("../release/tauri.updater.conf.json", import.meta.url)));
...
test("manual rehearsal retains the complete matrix without production credentials or publication", async () => {
  const workflow = await readFile(new URL("../.github/workflows/updater-artifact-rehearsal.yml", import.meta.url), "utf8");
