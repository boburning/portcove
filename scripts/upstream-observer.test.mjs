...
const config = JSON.parse(await readFile(new URL("../release/upstream-observer.json", import.meta.url), "utf8"));
...
const asset = (id = 2, contents = "redistributable fixture one") => ({ id, name: `fixture-${id}-windows.zip`, size: Buffer.byteLength(contents), state: "uploaded", digest: `sha256:${createHash("sha256").update(contents).digest("hex")}`, browser_download_url: `https://github.com/${config.repository}/releases/download/v1.0.0/fixture-${id}-windows.zip`, created_at: time, updated_at: time });

function fixture(overrides = {}) {
  const replies = new Map([
    [root, { body: { id: config.repository_id, full_name: config.repository, archived: false } }],
  ...
  const fetch = async (url, options) => {
    ...
    return new Response(JSON.stringify(reply.body), { headers: { "content-type": "application/json", etag, link: reply.link ?? "" } });
  };
...
test("records exact release and asset facts, conditional cache and changed fixture artifacts", async () => {
  ...
  const first = await observeUpstream(config, { fetch: server.fetch, token: "fixture-token" });
  ...
  assert.ok(server.requests.every(request => request.options.redirect === "error" && request.options.headers.Authorization === "Bearer fixture-token"));
  const second = await observeUpstream(config, { fetch: server.fetch, cache: first.cache });
  ...
  server.replies.get(`${root}/releases/1/assets?per_page=100&page=1`).body = [asset(2, "replacement bytes under the same asset identity")];
  const replaced = await observeUpstream(config, { fetch: server.fetch, cache: second.cache });
  ...
  const changed = await observeUpstream(config, { fetch: server.fetch, cache: replaced.cache });
...
test("traverses complete asset pagination and rejects partial, repeated or escaped pages", async () => {
  ...
  server.replies.set(page, { body: Array.from({ length: 100 }, (_, index) => asset(index + 1)), link: `<${next}>; rel="next"` });
  ...
  await assert.rejects(observeUpstream(config, { fetch: server.fetch }), error => error.rule === "pagination");
  server.replies.get(page).body = [asset(1)];
  await assert.rejects(observeUpstream(config, { fetch: server.fetch }), /nonterminal page is incomplete/);
  server.replies.get(page).link = '<https://foreign.invalid/private?per_page=100&page=2>; rel="next"';
  await assert.rejects(observeUpstream(config, { fetch: server.fetch }), /escaped its configured collection/);
});
...
test("partial reads and concurrent collection changes never advance the previous cache", async () => {
  ...
  const partial = fixture({ intercept: url => url.includes("/assets?") ? new Response(null, { status: 503 }) : null });
  await assert.rejects(observeUpstream(config, { fetch: partial.fetch, cache: baseline.cache, sleep: async () => {} }), error => error.rule === "provider-status");
...
test("rate limits defer to provider clocks and transport retries stay bounded", async () => {
  ...
  const delays = [];
  const broken = fixture({ intercept: () => { throw new Error("untrusted transport detail"); } });
  await assert.rejects(observeUpstream(config, { fetch: broken.fetch, sleep: async ms => delays.push(ms) }), error => error.rule === "transport" && !error.message.includes("untrusted"));
...
test("budgets, malformed metadata, cache corruption and repository drift fail closed", async () => {
  await assert.rejects(observeUpstream({ ...config, budget: { ...config.budget, requests: 2 } }, { fetch: fixture().fetch }), error => error.rule === "budget");
  await assert.rejects(observeUpstream({ ...config, budget: { ...config.budget, response_bytes: 1 } }, { fetch: fixture().fetch }), error => error.rule === "budget");
  const malformed = fixture();
  malformed.replies.get(root).body.id++;
  await assert.rejects(observeUpstream(config, { fetch: malformed.fetch }), error => error.rule === "repository-identity");
  ...
  await assert.rejects(observeUpstream(config, { fetch: fixture().fetch, cache: baseline.cache }), error => error.rule === "invalid-cache");
  assert.throws(() => validateObserverConfig({ ...config, arbitrary_scope: true }), /unexpected or missing fields/);
});

test("false prerelease flags and prerelease-only repositories remain observations for shared core policy", async () => {
  const server = fixture();
  server.replies.get(`${root}/releases?per_page=100&page=1`).body = [release(1, { tag_name: "1.1-rc5", prerelease: false })];
  const value = asset();
  ...
  assert.equal((await observeUpstream(config, { fetch: server.fetch })).observation.facts.releases.length, 1);
});

const project = async observation => ({ format: 1, port_id: config.port_id, repository_id: config.repository_id, facts_sha256: observation.facts_sha256, projections: [] });
...
test("one checkpoint coordinator owns writes and interrupted locks are never stolen", async t => {
  ...
  const directory = await mkdtemp(fileURLToPath(new URL("../work/observer-lock-test-", import.meta.url)));
  t.after(() => rm(directory, { recursive: true }));
  const state = path.join(directory, "checkpoint.json");
  await withCheckpointLock(state, async () => {
    await assert.rejects(withCheckpointLock(state, () => assert.fail("second writer entered")), { code: "EEXIST" });
  });
  await assert.rejects(withCheckpointLock(state, () => { throw new Error("interrupted operation"); }), /interrupted operation/);
  ...
  await assert.rejects(withCheckpointLock(state, () => assert.fail("stale lock stolen")), { code: "EEXIST" });
...
test("checkpoints preserve the last complete snapshot, deduplicate failures and expose stale unmonitored scope", async () => {
  ...
  const options = { fetch: fixture().fetch, project, now: () => clock, sleep: async () => {}, catalogIds: [config.port_id, "unmonitored-port"] };
  ...
  clock += 13 * 3_600_000;
  const broken = fixture({ intercept: () => new Response(null, { status: 503 }) });
  const failed = await advanceObservation(config, first.checkpoint, { ...options, fetch: broken.fetch });
  ...
  const repeated = await advanceObservation(config, failed.checkpoint, { ...options, fetch: broken.fetch });
...
test("deferred checkpoints honor retry clocks without requests and mismatched core output cannot advance", async () => {
  ...
  assert.equal(limited.requests.length, 1);
  clock += 120_000;
  const good = await advanceObservation(config, second.checkpoint, { ...options, fetch: fixture().fetch });
