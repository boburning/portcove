...
async function collectDesktopPackages(
  projectRoot,
  bundleRoot,
  expectedEntries,
) {
  ...
  for (const format of formats) {
    ...
    await assertOwnedUnlinkedPath(
      projectRoot,
      formatRoot,
      `release ${format} bundle directory`,
    );
    for (const entry of await readdir(formatRoot, { withFileTypes: true })) {
      ...
      if (entry.isFile() && isDesktopPackage(entryPath))
...
export function validateStageRoot(projectRoot, stageRoot) {
  ...
  if (
    stage === path.parse(stage).root ||
    stage === project ||
    !contains(project, stage)
  ) {
    throw new Error(
      "release staging path must be a child directory inside the project",
    );
  }
  if (
    sourceRoots.some(
      (source) => contains(source, stage) || contains(stage, source),
    )
  ) {
...
export async function collectReleaseArtifacts(
  ...
  await assertOwnedUnlinkedPath(
    projectRoot,
    bundleRoot,
    "release desktop bundle directory",
  );
  const desktopPackages = await collectDesktopPackages(
    projectRoot,
    bundleRoot,
    expectedEntries,
  );
...
function parseArguments(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (
      !["--label", "--project-root", "--stage-dir", "--version"].includes(name)
    )
...
async function main() {
  const { projectRoot, label, stageRoot, version } = parseArguments(
  ...
  if (stageRoot) {
    ...
    console.log(
      `Wrote ${path.basename(output)} for ${artifacts.length} release artifacts.`,
    );
  }
}
