...
function makeInstallerLifecycleFixture(t) {
  ...
  t.after(() => {
    spawnSync("reg.exe", ["delete", `HKCU\\${keyPath}`, "/f"], { windowsHide: true });
    rmSync(root, { recursive: true, force: true, maxRetries: 20, retryDelay: 100 });
  });

  const desktopSource = path.join(root, "desktop.cs");
  writeFileSync(desktopSource, String.raw`using System; using System.IO; using System.Runtime.InteropServices; using System.Windows.Forms;
...
function removeInstallerLifecycleRegistration(item) {
  spawnSync("reg.exe", ["delete", `HKCU\\${item.keyPath}`, "/f"], { windowsHide: true });
}
...
`);
  ...
  execFileSync("git", ["config", "user.name", "Portcove test"], { cwd: repository, windowsHide: true });
  execFileSync("git", ["config", "user.email", "test@portcove.invalid"], { cwd: repository, windowsHide: true });
  execFileSync("git", ["add", "."], { cwd: repository, windowsHide: true });
  execFileSync("git", ["commit", "--quiet", "-m", "fixture candidate"], { cwd: repository, windowsHide: true });
...
function prepareArgs(item, validateOnly = false) {
  const args = ["-Action", "prepare", "-SessionRoot", item.session, "-CandidateCheckout", item.repository, "-BuildRecordPath", item.record, "-ExpectedBuildRecordSha256", item.recordHash];
...
test("stateful fake packaged session prepares, journals process identity, checkpoints, relaunches, and finishes", { skip: process.platform !== "win32", timeout: 180_000 }, t => {
  const item = makeFixture(t);
  ...
  assert.equal(sha256(path.join(item.session, state.finish_receipt.pre_finish_session_path)), state.finish_receipt.pre_finish_session_sha256);
...
test("a checkpoint with changed evidence is rejected before another action", { skip: process.platform !== "win32", timeout: 180_000 }, t => {
  const item = makeFixture(t);
  const prepared = runPowerShell(prepareArgs(item));
  assert.equal(prepared.status, 0, prepared.stderr);
...
test("an interrupted prepare binds its journal and runs only a correctly hashed harmless uninstaller", { skip: process.platform !== "win32", timeout: 120_000 }, t => {
  const item = makeFixture(t);
  const failed = runPowerShell(prepareArgs(item), { env: { ...process.env, PORTCOVE_FIXTURE_FAIL: "1", PORTCOVE_FIXTURE_UNINSTALLER: item.uninstaller, PORTCOVE_FIXTURE_UNINSTALLER_HASH: "correct" } });
  assert.notEqual(failed.status, 0);
  let state = JSON.parse(readFileSync(path.join(item.session, "session.json"), "utf8"));
  assert.equal(state.phase, "prepare_failed");
  const installerEvidence = JSON.parse(readFileSync(path.join(item.session, "evidence", "installer-lifecycle.json"), "utf8"));
  ...
  assert.equal(state.files.installer_evidence.sha256, sha256(path.join(item.session, "evidence", "installer-lifecycle.json")));
  ...
  assert.equal(existsSync(path.join(item.session, "installer-work", "run-fixture", "installed", "uninstall.exe")), false);
});

for (const mode of ["missing", "wrong"]) {
  test(`abort retains evidence when the uninstaller hash is ${mode}`, { skip: process.platform !== "win32", timeout: 120_000 }, t => {
    const item = makeFixture(t);
    const failed = runPowerShell(prepareArgs(item), { env: { ...process.env, PORTCOVE_FIXTURE_FAIL: "1", PORTCOVE_FIXTURE_UNINSTALLER: item.uninstaller, PORTCOVE_FIXTURE_UNINSTALLER_HASH: mode } });
    assert.notEqual(failed.status, 0);
    const aborted = runPowerShell(["-Action", "abort", "-SessionRoot", item.session]);
    assert.notEqual(aborted.status, 0);
    assert.match(aborted.stderr, mode === "missing" ? /uninstaller was not hash-journaled/ : /partial-run uninstaller SHA-256 mismatch/);
    const state = JSON.parse(readFileSync(path.join(item.session, "session.json"), "utf8"));
    assert.equal(state.phase, "prepare_failed");
    assert.ok(existsSync(path.join(item.session, "installer-work", "run-fixture", "installed", "uninstall.exe")));
  });
}

test("abort refuses an installer journal that was never hash-bound after a hard interruption", { skip: process.platform !== "win32", timeout: 120_000 }, t => {
  const item = makeFixture(t);
  const failed = runPowerShell(prepareArgs(item), { env: { ...process.env, PORTCOVE_FIXTURE_FAIL: "1", PORTCOVE_FIXTURE_UNINSTALLER: item.uninstaller, PORTCOVE_FIXTURE_UNINSTALLER_HASH: "correct" } });
  assert.notEqual(failed.status, 0);
...
test("abort refuses success when a hash-verified uninstaller leaves managed install files", { skip: process.platform !== "win32", timeout: 120_000 }, t => {
  const item = makeFixture(t);
  const environment = { ...process.env, PORTCOVE_FIXTURE_FAIL: "1", PORTCOVE_FIXTURE_UNINSTALLER: item.uninstaller, PORTCOVE_FIXTURE_UNINSTALLER_HASH: "correct", PORTCOVE_FIXTURE_UNINSTALL_LEAVE: "1" };
  assert.notEqual(runPowerShell(prepareArgs(item), { env: environment }).status, 0);
  const aborted = runPowerShell(["-Action", "abort", "-SessionRoot", item.session], { env: environment });
  assert.notEqual(aborted.status, 0);
...
test("a killed abort runner resumes only after exact uninstaller absence and owned cleanup are proven", { skip: process.platform !== "win32", timeout: 180_000 }, async t => {
  const item = makeFixture(t);
  const baseEnvironment = { ...process.env, PORTCOVE_FIXTURE_FAIL: "1", PORTCOVE_FIXTURE_UNINSTALLER: item.uninstaller, PORTCOVE_FIXTURE_UNINSTALLER_HASH: "correct" };
  assert.notEqual(runPowerShell(prepareArgs(item), { env: baseEnvironment }).status, 0);
  const child = spawn("pwsh.exe", ["-NoLogo", "-NoProfile", "-File", script, "-Action", "abort", "-SessionRoot", item.session], { windowsHide: true, env: { ...baseEnvironment, PORTCOVE_FIXTURE_UNINSTALL_DELAY_MS: "1800" }, stdio: "ignore" });
  const sessionPath = path.join(item.session, "session.json");
  let state;
  ...
  execFileSync("taskkill.exe", ["/PID", String(child.pid), "/F"], { windowsHide: true, stdio: "ignore" });
  const uninstallerPath = path.join(item.session, "installer-work", "run-fixture", "installed", "uninstall.exe");
  const cleanupDeadline = Date.now() + 15_000;
  while (existsSync(uninstallerPath) && Date.now() < cleanupDeadline) await new Promise(resolve => setTimeout(resolve, 100));
  assert.equal(existsSync(uninstallerPath), false);
  const resumed = runPowerShell(["-Action", "abort", "-SessionRoot", item.session], { env: baseEnvironment });
...
test("a runner killed after an observed zero exit proves quiescence without relaunch", { skip: process.platform !== "win32", timeout: 180_000 }, async t => {
  const item = makeFixture(t);
  const baseEnvironment = { ...process.env, PORTCOVE_FIXTURE_FAIL: "1", PORTCOVE_FIXTURE_UNINSTALLER: item.uninstaller, PORTCOVE_FIXTURE_UNINSTALLER_HASH: "correct" };
  assert.notEqual(runPowerShell(prepareArgs(item), { env: baseEnvironment }).status, 0);
  const child = spawn("pwsh.exe", ["-NoLogo", "-NoProfile", "-File", script, "-Action", "abort", "-SessionRoot", item.session], { windowsHide: true, env: { ...baseEnvironment, PORTCOVE_QUALIFICATION_TEST_PAUSE_AFTER_ABORT_EXIT_MS: "5000" }, stdio: "ignore" });
  const sessionPath = path.join(item.session, "session.json");
  let state;
  const deadline = Date.now() + 15_000;
  while (Date.now() < deadline) {
...
test("an observed nonzero abort attempt remains failed and is not relaunched", { skip: process.platform !== "win32", timeout: 180_000 }, t => {
  const item = makeFixture(t);
  const environment = { ...process.env, PORTCOVE_FIXTURE_FAIL: "1", PORTCOVE_FIXTURE_UNINSTALLER: item.uninstaller, PORTCOVE_FIXTURE_UNINSTALLER_HASH: "correct", PORTCOVE_FIXTURE_UNINSTALL_EXIT: "7" };
  assert.notEqual(runPowerShell(prepareArgs(item), { env: environment }).status, 0);
  const first = runPowerShell(["-Action", "abort", "-SessionRoot", item.session], { env: environment });
  assert.notEqual(first.status, 0);
  const second = runPowerShell(["-Action", "abort", "-SessionRoot", item.session], { env: environment });
...
test("a killed real lifecycle runner leaves readable inner WAL and outer ambiguity fails closed", { skip: process.platform !== "win32", timeout: 180_000 }, async t => {
  const item = makeFixture(t);
  const failureEnvironment = { ...process.env, PORTCOVE_FIXTURE_FAIL: "1", PORTCOVE_FIXTURE_UNINSTALLER: item.uninstaller, PORTCOVE_FIXTURE_UNINSTALLER_HASH: "correct" };
  ...
  try { execFileSync("taskkill.exe", ["/PID", String(evidence.process_runs[0].pid), "/F"], { windowsHide: true, stdio: "ignore" }); } catch {}
});

test("launch-pending recovery refuses completion when no unique exact process exists", { skip: process.platform !== "win32", timeout: 180_000 }, t => {
  const item = makeFixture(t);
  const prepared = runPowerShell(prepareArgs(item));
  assert.equal(prepared.status, 0, prepared.stderr);
  const sessionPath = path.join(item.session, "session.json");
  ...
  const pending = { ...running, id: "pending-fixture", requested_at: new Date().toISOString(), status: "launch_pending", pid: null, start_time: null, start_time_filetime: null, window_title: null, exit_code: null, exit_observation: null };
  state.process_runs.push(pending);
  state.active_run_id = pending.id;
  writeFileSync(sessionPath, `${JSON.stringify(state, null, 2)}\n`);
  const finish = runPowerShell(["-Action", "finish", "-SessionRoot", item.session]);
  assert.notEqual(finish.status, 0);
  assert.match(finish.stderr, /launch-pending run could not be bound to one exact desktop process/);
  assert.equal(JSON.parse(readFileSync(sessionPath, "utf8")).phase, "prepared");
});

test("launch-pending recovery rejects a matching process started before the request tolerance", { skip: process.platform !== "win32", timeout: 180_000 }, async t => {
  const item = makeFixture(t);
  const prepared = runPowerShell(prepareArgs(item));
  assert.equal(prepared.status, 0, prepared.stderr);
  const sessionPath = path.join(item.session, "session.json");
...
for (const faultpoint of ["after_snapshot_temp", "after_snapshot", "after_receipt_temp", "after_receipt"]) {
  test(`finish resumes the same attempt after ${faultpoint}`, { skip: process.platform !== "win32", timeout: 180_000 }, t => {
    const item = makeFixture(t);
    const prepared = runPowerShell(prepareArgs(item));
    assert.equal(prepared.status, 0, prepared.stderr);
    const interrupted = runPowerShell(["-Action", "finish", "-SessionRoot", item.session], { env: { ...process.env, PORTCOVE_QUALIFICATION_TEST_FAULTPOINT: faultpoint } });
    assert.notEqual(interrupted.status, 0);
    const sessionPath = path.join(item.session, "session.json");
    let state = JSON.parse(readFileSync(sessionPath, "utf8"));
    assert.equal(state.phase, "finish_pending");
    const expectedStage = faultpoint === "after_snapshot_temp" ? "pending" : faultpoint === "after_snapshot" || faultpoint === "after_receipt_temp" ? "snapshot_written" : "receipt_written";
    ...
    if (faultpoint === "after_snapshot_temp") {
      const attemptDirectory = path.dirname(path.join(item.session, state.finish_attempt.snapshot_path));
      const temporary = readdirSync(attemptDirectory).find(name => name.startsWith("pre-finish-session.json.tmp-"));
      assert.ok(temporary);
...
test("generator refuses to overwrite an immutable build record", { skip: process.platform !== "win32", timeout: 120_000 }, t => {
  const item = makeFixture(t);
  const record = JSON.parse(readFileSync(item.record, "utf8"));
  const args = [recordTool, "--repository", item.repository,
    "--installer", path.join(item.repository, record.artifacts.installer.relative_path),
    "--cli", path.join(item.repository, record.artifacts.cli.relative_path),
    "--desktop", path.join(item.repository, record.artifacts.desktop.relative_path),
    "--predecessor", path.join(item.repository, record.predecessor.installer.relative_path),
    "--predecessor-version", record.predecessor.version, "--output", item.record];
