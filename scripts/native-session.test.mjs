...
const script = fileURLToPath(new URL("../apps/desktop/scripts/native-session.ps1", import.meta.url));
const windows = { skip: process.platform !== "win32", timeout: 20_000 };
const run = (mode, snapshot) => spawnSync("pwsh", ["-NoProfile", "-File", script, "-Mode", mode, "-DriverProcessId", String(process.pid), "-ApplicationPath", process.execPath, "-SnapshotPath", snapshot], { encoding: "utf8", windowsHide: true, timeout: 10_000 });
...
test("native shutdown wait is bounded, preserves a live process, and accepts its later exit", windows, async () => {
  ...
  try {
    ...
    for (const record of recorded.processes) record.started_filetime = (BigInt(record.started_filetime) - 10_000n).toString();
    const stale = path.join(root, "stale.json"); await writeFile(stale, JSON.stringify(recorded));
    const staleResult = run("Wait", stale);
    assert.equal(staleResult.status, 0, `A reused PID must not be treated as the captured process: ${staleResult.stderr}; ${JSON.stringify(recorded)}`);
    process.kill(child.pid, 0);
    await stop(child);
    const ended = run("Wait", snapshot);
    assert.equal(ended.status, 0, ended.stderr);
  } finally { await stop(child); await rm(root, { recursive: true, force: true }); }
});
