...
async function fixture(t, label = "macos-aarch64") {
  ...
  await copyFile(
    new URL("../release/package-policy.json", import.meta.url),
    path.join(root, "release/package-policy.json"),
  );
  await mkdir(path.join(root, "target/release/bundle/macos/Portcove.app/Contents/Resources"), {
    recursive: true,
  });
  for (const entry of packagesForPlatform(policy, label)) {
    ...
    await writeFile(
      path.join(root, rootName, artifactName(entry, fixtureVersion)),
      `${entry.interface}:${entry.id}`,
    );
  }
  await writeFile(
    path.join(root, "target/release/bundle/macos/Portcove.app/Contents/Resources/icon.icns"),
    "internal",
  );
  return root;
}
...
test("hashes distributable packages and ignores internal application files", async (t) => {
  ...
  assert.deepEqual(
    result.artifacts.map((artifact) => path.basename(artifact)),
    ["Portcove_0.1.0-alpha.2_aarch64.dmg", "portcove-cli-0.1.0-alpha.2-macos-aarch64.tar.gz"],
  );
  const manifest = await readFile(result.output, "utf8");
  assert.match(manifest, /^[a-f0-9]{64} {2}portcove-cli-0\.1\.0-alpha\.2-macos-aarch64\.tar\.gz$/m);
  assert.match(manifest, /^[a-f0-9]{64} {2}Portcove_0\.1\.0-alpha\.2_aarch64\.dmg$/m);
...
test("ignores internal AppImage links while selecting direct policy-declared packages", async (t) => {
  ...
  assert.deepEqual(
    result.map((artifact) => path.basename(artifact)).sort(),
    [
...
test("rejects unsafe or overlapping release staging paths", async (t) => {
  ...
  await assert.rejects(
    stageReleaseArtifacts(
      root,
      "macos-aarch64",
      path.join(root, "target/release/bundle/upload"),
      fixtureVersion,
    ),
...
test("refuses staging through linked ancestry without modifying the link target", async (t) => {
  ...
  await assert.rejects(
    stageReleaseArtifacts(
      root,
      "macos-aarch64",
      path.join(linkedParent, "release-upload"),
      fixtureVersion,
    ),
  ...
  await assert.rejects(
    readFile(path.join(outside, "release-upload", "SHA256SUMS-macos-aarch64.txt")),
...
test("rejects duplicate package filenames before publishing", async (t) => {
  const root = await fixture(t, "linux-x86_64");
  await writeFile(
    path.join(root, "target/release/bundle/rpm/Portcove_0.1.0-alpha.2_amd64.deb"),
    "duplicate",
  );
  await assert.rejects(
...
test("fails closed for missing, unexpected, or wrong-version packages", async (t) => {
  ...
  const cliArchive = path.join(
    root,
    "release-assets/portcove-cli-0.1.0-alpha.2-windows-x86_64.zip",
  );
  await rm(cliArchive);
  await assert.rejects(
    collectReleaseArtifacts(root, "windows-x86_64", fixtureVersion),
    /missing: portcove-cli/,
  );

  await writeFile(cliArchive, "cli");
  await writeFile(
    path.join(root, "target/release/bundle/nsis/Portcove_0.1.0-alpha.2_x64.msi"),
    "unexpected",
  );
  await assert.rejects(
    collectReleaseArtifacts(root, "windows-x86_64", fixtureVersion),
  ...
  await assert.rejects(
    collectReleaseArtifacts(root, "windows-x86_64", "0.1.0-alpha.3"),
