...
async function fixture(t, label = "macos-aarch64") {
  ...
  await mkdir(
    path.join(
      root,
      "target/release/bundle/macos/Portcove.app/Contents/Resources",
    ),
    { recursive: true },
  );
  ...
  await writeFile(
    path.join(
      root,
      "target/release/bundle/macos/Portcove.app/Contents/Resources/icon.icns",
    ),
    "internal",
  );
  return root;
...
test("ignores internal AppImage links while selecting direct policy-declared packages", async (t) => {
  ...
  const appDirectory = path.join(
    root,
    "target/release/bundle/appimage/Portcove.AppDir",
  );
...
test("refuses to overwrite an existing release staging directory", async (t) => {
  ...
  assert.equal(
    await readFile(path.join(stage, "existing.txt"), "utf8"),
    "preserve",
  );
});
...
test("refuses staging through linked ancestry without modifying the link target", async (t) => {
  ...
  const outside = await mkdtemp(
    path.join(os.tmpdir(), "portcove-stage-outside-"),
  );
  ...
  await assert.rejects(
    readFile(
      path.join(outside, "release-upload", "SHA256SUMS-macos-aarch64.txt"),
    ),
...
test("rejects duplicate package filenames before publishing", async (t) => {
  ...
  await writeFile(
    path.join(
      root,
      "target/release/bundle/rpm/Portcove_0.1.0-alpha.2_amd64.deb",
    ),
    "duplicate",
  );
  await assert.rejects(
...
test("fails closed for missing, unexpected, or wrong-version packages", async (t) => {
  ...
  await writeFile(
    path.join(
      root,
      "target/release/bundle/nsis/Portcove_0.1.0-alpha.2_x64.msi",
    ),
    "unexpected",
  );
  await assert.rejects(
