...
const definition = { id: "example", command: ["example", "--version"], version: "1.2.3" };
test("doctor distinguishes exact, mismatched, failed and absent tools without raw output", () => {
  ...
  const missing = probeTool(definition, () => { throw Object.assign(new Error("SECRET"), { code: "ENOENT" }); });
  assert.equal(missing.status, "unavailable");
  assert.ok(!JSON.stringify(missing).includes("SECRET"));
});
test("doctor reports a timeout rather than a version pass", () => {
  assert.equal(probeTool(definition, () => ({ status: null, error: { code: "ETIMEDOUT" }, stdout: "1.2.3" })).status, "timeout");
});
