...
const platformIdentities = new Map([
  ["windows-x86_64", { os: "windows", architecture: "x86_64", desktopFormats: new Set(["nsis"]), cliFormat: "zip" }],
  ["linux-x86_64", { os: "linux", architecture: "x86_64", desktopFormats: new Set(["appimage", "deb", "rpm"]), cliFormat: "tar.gz" }],
  ["macos-aarch64", { os: "macos", architecture: "aarch64", desktopFormats: new Set(["dmg"]), cliFormat: "tar.gz" }],
  ["macos-x86_64", { os: "macos", architecture: "x86_64", desktopFormats: new Set(["dmg"]), cliFormat: "tar.gz" }],
]);
...
export function validatePackagePolicy(policy) {
  ...
  if (policy?.repository !== "boburning/portcove") errors.push("package policy repository must be boburning/portcove");
  if (policy?.checksum_manifest !== "SHA256SUMS.txt") errors.push("public checksum manifest must be SHA256SUMS.txt");
  ...
  for (const [index, entry] of policy.packages.entries()) {
    ...
    if (!safeIdentifier(entry?.id) || ids.has(entry.id)) errors.push(`missing, unsafe, or duplicate package id: ${label}`);
    if (entry?.id) ids.add(entry.id);
    if (!allowedInterfaces.has(entry?.interface)) errors.push(`${label} has an invalid interface`);
    if (!allowedPlatforms.has(entry?.platform_label)) errors.push(`${label} has an invalid platform_label`);
    if (!allowedArchitectures.has(entry?.architecture)) errors.push(`${label} has an invalid architecture`);
    ...
    const formatMatches = entry?.interface === "desktop"
    ...
    if (identity && allowedInterfaces.has(entry?.interface) && !formatMatches) {
      errors.push(`${label} format ${entry?.format ?? "missing"} does not match ${entry.platform_label} ${entry.interface}`);
    }
    if (typeof entry?.experimental !== "boolean") errors.push(`${label} experimental must be boolean`);
    if (!safeFilenameTemplate(entry?.filename)) {
      ...
      if (filenames.has(normalized)) errors.push(`duplicate package filename template: ${entry.filename}`);
...
export async function loadPackagePolicy(projectRoot = defaultProjectRoot) {
  const policy = JSON.parse(await readFile(path.join(projectRoot, "release", "package-policy.json"), "utf8"));
  const errors = validatePackagePolicy(policy);
  if (errors.length) throw new Error(`invalid release package policy:\n${errors.map(error => `- ${error}`).join("\n")}`);
  return policy;
}

export function artifactName(entry, version) {
  if (!semverPattern.test(version ?? "")) throw new Error(`invalid release version: ${version ?? "missing"}`);
...
export function assertExactArtifactNames(expected, actual, context) {
  ...
  for (const name of actual) {
    ...
    if (seen.has(key)) throw new Error(`${context} contains a duplicate or case-colliding filename: ${name}`);
    seen.add(key);
  }
  ...
  if (JSON.stringify(expectedSorted) !== JSON.stringify(actualSorted)) {
    ...
    throw new Error(`${context} mismatch; missing: ${missing.join(", ") || "none"}; unexpected: ${unexpected.join(", ") || "none"}`);
  }
}

export function packagesForPlatform(policy, platformLabel) {
  if (!allowedPlatforms.has(platformLabel)) throw new Error(`unknown release platform: ${platformLabel}`);
...
export async function workspaceVersion(projectRoot = defaultProjectRoot) {
  ...
  if (!semverPattern.test(version ?? "")) throw new Error(`Cargo workspace version is invalid: ${version ?? "missing"}`);
  return version;
}
...
async function main() {
  ...
  if (entries.length !== 1) throw new Error(`expected exactly one matching package; found ${entries.length}`);
