...
const base = path.resolve(
  process.env.RUNNER_TEMP || process.env.PORTCOVE_TEMP_DIR || tmpdir(),
);
...
try {
  ...
  if (compiled.status !== 0)
    throw new Error("Host-tool fixture compilation failed");
  const args = process.argv.slice(2);
  if (args.length === 1 && args[0] === "--prepare-only") {
    if (!process.env.GITHUB_ENV)
      throw new Error("--prepare-only requires GITHUB_ENV");
    appendFileSync(
